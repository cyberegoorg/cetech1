...
pub fn registerToApi() !void {
    try apidb.setZigApi(module_name, host.MonitorApi, &monitor_api);
    try apidb.setZigApi(module_name, host.WindowApi, &window_api);

    try apidb.implInterface(module_name, input.InputSourceI, &keyboard_input_source);
    try apidb.implInterface(module_name, input.InputSourceI, &mouse_input_source);
    try apidb.implInterface(module_name, input.InputSourceI, &gamepad_input_source);
}

...
const instance_system = cetech1.renderer_pipeline.instance_system;
const editor = cetech1.editor;

const public = cetech1.renderer_pipeline.render_component;
    ...
    struct {
        ...
        pub fn tick(world: *ecs.World, it: *ecs.Iter, dt: f32) !void {
            ...
            if (all_instances.items.len > 0) {
                ...
                try graphvm.setInstancesContext(all_instances.items, ecs.ECS_WORLD_CONTEXT, world);
...
const update_render_component_system_i = ecs.SystemI.implement(
    ...
    struct {
        ...
        //     // log.debug("render component Tick {d}", .{all_instances.items.len});
        //     try graphvm.setInstancesContext(instances, ecs.ECS_WORLD_CONTEXT, world);
...
fn submitDrawcall(
    ...
    const shader_io = shader_system.getShaderIO(dc.shader.?);

    if (dc.uniforms) |u| shader_system.bindConstant(shader_io, u, e);
    if (dc.resouces) |r| shader_system.bindResource(shader_io, r, e);
    ...
    struct {
        ...
        pub fn render(
...

            {
                ...
                // TODO: SHIT
                try graphvm.setInstancesContext(containers.items, ecs.ECS_WORLD_CONTEXT, world);
            }
            ...
            for (clusters) |cluster| {
                if (gpu_backend.getEncoder()) |e| {
                    ...
                    if (cluster.calls[0]) |dc| {
                        ...
                        var shader_context = try shader_system.cloneSystemContext(system_context.*);
                        defer shader_system.destroySystemContext(shader_context);

...
const public = cetech1.renderer.viewport;
...
pub const api = public.RenderViewportApi{
    ...
    .frezeMainCameraCulling = frezeMainCameraCulling,
    .getDebugCulling = getDebugCulling,
    .getMainCamera = getMainCamera,
    .getSize = getSize,
    .getTexture = getTexture,
    .requestRender = requestRender,
    .setDebugCulling = setDebugCulling,
    .setMainCamera = setMainCamera,
    .setSelectedEntity = setSelectedEntity,
    .setSize = setSize,
};

pub fn setSize(viewport: *public.Viewport, size: math.Vec2f) void {
    const true_viewport: *Viewport = @ptrCast(@alignCast(viewport));
    true_viewport.new_size.x = @max(size.x, 1);
    true_viewport.new_size.y = @max(size.y, 1);
}

pub fn getTexture(viewport: *public.Viewport) ?gpu.TextureHandle {
    const true_viewport: *Viewport = @ptrCast(@alignCast(viewport));
...
pub fn getSize(viewport: *public.Viewport) math.Vec2f {
    const true_viewport: *Viewport = @ptrCast(@alignCast(viewport));
    return true_viewport.size;
}

pub fn requestRender(viewport: *public.Viewport) void {
    const true_viewport: *Viewport = @ptrCast(@alignCast(viewport));
    true_viewport.renderMe = true;
}

pub fn setMainCamera(viewport: *public.Viewport, camera_ent: ?ecs.EntityId) void {
    const true_viewport: *Viewport = @ptrCast(@alignCast(viewport));
    true_viewport.main_camera_entity = camera_ent;
}

pub fn getMainCamera(viewport: *public.Viewport) ?ecs.EntityId {
    const true_viewport: *Viewport = @ptrCast(@alignCast(viewport));
    return true_viewport.main_camera_entity;
}

pub fn getDebugCulling(viewport: *public.Viewport) bool {
    const true_viewport: *Viewport = @ptrCast(@alignCast(viewport));
    return true_viewport.renderables_culling.draw_culling_sphere_debug;
}

pub fn setDebugCulling(viewport: *public.Viewport, enable: bool) void {
    const true_viewport: *Viewport = @ptrCast(@alignCast(viewport));
...
pub fn setSelectedEntity(viewport: *public.Viewport, entity: ?ecs.EntityId) void {
    const true_viewport: *Viewport = @ptrCast(@alignCast(viewport));
    true_viewport.selected_entity = entity;
}

pub fn frezeMainCameraCulling(viewport: *public.Viewport, freeze: bool) void {
    const true_viewport: *Viewport = @ptrCast(@alignCast(viewport));
    if (freeze) {
...
const RenderViewportTask = struct {
    ...
    pub fn exec(self: *@This()) !void {
        ...
        if (@intFromPtr(render_module) == 0) return;

        const viewport: *Viewport = self.viewport;
        const vp: *public.Viewport = @ptrCast(viewport);

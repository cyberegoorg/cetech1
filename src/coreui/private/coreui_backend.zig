...
const log = std.log.scoped(.coreui_backend);
...
pub fn deinit(gpu_backend: ?gpu.GpuBackend) void {
    if (backend_init) {
        if (gpu_backend) |api| {
            const iotext = zgui.platform_io.getTextures();
            const textures = iotext.items[0..@intCast(iotext.len)];
            for (textures) |texture| {
                const texture_handle = gpu.TextureHandle{ .idx = @intCast(@intFromEnum(texture.tex_id)) };
                api.destroyTexture(texture_handle);
            }
        }
        ...
        for (textures) |texture| {
            switch (texture.status) {
                ...
                .want_create => {
                    ...
                    gpu_api.setTextureName(new_tex, "CoreUI texture");

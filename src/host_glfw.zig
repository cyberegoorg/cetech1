...
pub fn createWindow(width: i32, height: i32, title: [:0]const u8, monitor: ?host.Monitor) !host.Window {
    ...
    _ = zglfw.setKeyCallback(w, keyCallback);
    _ = zglfw.setCursorPosCallback(w, cursorPosCallback);
    _ = zglfw.setMouseButtonCallback(w, mouseButtonCallback);
    _ = zglfw.setFramebufferSizeCallback(w, framebufferSizeCallback);
    _ = zglfw.setWindowContentScaleCallback(w, contentScaleCallback);

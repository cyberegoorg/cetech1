...
const assetdb_private = @import("../../kernel/private/assetdb.zig");
...
const World = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, w: *zflecs.world_t) !World {
        var zz = profiler.ZoneN(@src(), "ECS: init self world");
        defer zz.End();
    ...
    pub fn deinit(self: *World) void {
        for (_component_by_id.values()) |iface| {
            if (iface.destroy_manager) |destroyManager| {
    ...
    pub fn progress(self: *World, dt: f32) bool {
        var zone_ctx = profiler.ZoneN(@src(), "ECS: World progress");
        defer zone_ctx.End();
    ...
    pub fn createQuery(self: *World, query: public.QueryDesc) !*public.Query {
        ...
        if (query.order_by_component) |c| {
            ...
            qd.order_by_callback = query.order_by_callback;
        }
    ...
    pub fn debuguiMenuItems(self: *World, allocator: std.mem.Allocator) void {
        ...
        if (coreui.menuItem(allocator, if (!is_simulate) coreui.Icons.Play else coreui.Icons.Pause, .{}, null)) {
            self.setSimulate(!is_simulate);
        }

        {
            coreui.beginDisabled(.{ .disabled = is_simulate });
            defer coreui.endDisabled();

            if (coreui.menuItem(allocator, coreui.Icons.ForwardStep, .{}, null)) {
                self.doStep();
            }
        }
    }
    ...
    pub fn uiRemoteDebugMenuItems(self: *World, allocator: std.mem.Allocator, port: ?u16) ?u16 {
        ...
        if (coreui.beginMenu(allocator, coreui.Icons.Entity ++ "  " ++ "ECS", true, null)) {
            ...
            if (coreui.menuItem(allocator, copy_label, .{ .enabled = port != null }, null)) {
...
var sync_changes_from_cdb_task = cetech1.kernel.KernelTaskUpdateI.implment(
    ...
    struct {
        pub fn update(kernel_tick: u64, dt: f32) !void {
            ...
            {
                const changed = try cdb.getChangeObjects(alloc, db, public.EntityCdb.typeIdx(db), _entity_version);
                defer alloc.free(changed.objects);
                if (!changed.need_fullscan) {
                    for (changed.objects) |entity_obj| {
                        const is_alive = cdb.isAlive(entity_obj);
                        if (!is_alive) {
                    ...
                    //         const parent_obj = cdb.getParent(entity_obj);
                    ...
                    if (cdb.getAllObjectByType(alloc, db, public.EntityCdb.typeIdx(db))) |objs| {
                        for (objs) |graph| {
                            _ = graph;
                        }
                    }
            ...
            // Components
            //
            const impls = try apidb.getImpl(_allocator, public.ComponentI);
            defer _allocator.free(impls);
            for (impls) |iface| {
                if (iface.cdb_type_hash.isEmpty()) continue;
                const type_idx = cdb.getTypeIdx(db, iface.cdb_type_hash).?;

                const last_check = _component_version.get(type_idx) orelse 0;

                const changed = try cdb.getChangeObjects(alloc, db, type_idx, last_check);
                defer alloc.free(changed.objects);
                if (!changed.need_fullscan) {
                    for (changed.objects) |component_obj| {
                        const is_alive = cdb.isAlive(component_obj);
                        ...
                        for (worlds) |world| {
                            ...
                            const entity_obj = parent_obj orelse cdb.getParent(component_obj);
                            const prefab_ent = world.obj2prefab.get(entity_obj) orelse continue;
                            const deleted_entity = !cdb.isAlive(entity_obj);
                    ...
                    if (cdb.getAllObjectByType(alloc, db, type_idx)) |objs| {
                        for (objs) |graph| {
                            _ = graph;
                        }
                    }
...
var update_task = cetech1.kernel.KernelTaskUpdateI.implment(
    ...
    struct {
        pub fn update(kernel_tick: u64, dt: f32) !void {
            ...
            const component_category_i_version = apidb.getInterafcesVersion(public.ComponentCategoryI);
            if (component_category_i_version != _component_category_i_version) {
                log.debug("Supported components categories:", .{});
                const impls = try apidb.getImpl(allocator, public.ComponentCategoryI);
            ...
            const component_i_version = apidb.getInterafcesVersion(public.ComponentI);
            if (component_i_version != _component_i_version) {
                log.debug("Supported components:", .{});
                const impls = try apidb.getImpl(allocator, public.ComponentI);
                defer allocator.free(impls);
                for (impls) |iface| {
            ...
            // Entity
            {
                _ = try cdb.addType(
                    ...
                    &[_]cdb.PropDef{
                        ...
                        .{ .prop_idx = public.EntityCdb.propIdx(.Storage), .name = "storage", .type = cdb.PropType.STR },
...
pub fn registerToApi() !void {
    ...
    try apidb.implOrRemove(module_name, cetech1.kernel.KernelTaskUpdateI, &update_task, true);
}
...
fn getOrCreatePrefab(
    ...
    const top_level_obj_r = public.EntityCdb.read(obj).?;
    const name = public.EntityCdb.readStr(top_level_obj_r, .Name);
    ...
    // Create components
    if (try public.EntityCdb.readSubObjSet(top_level_obj_r, .Components, allocator)) |components| {
        defer allocator.free(components);

        for (components) |component_obj| {
            const component_hash = cdb.getTypeHash(db, component_obj.type_idx).?;

            try w.obj2parent_obj.put(_allocator, component_obj, obj);

            //log.debug("Create component {s}", .{cdb.getTypeName(db, component_obj.type_idx).?});
    ...
    if (try public.EntityCdb.readSubObjSet(top_level_obj_r, .Childrens, allocator)) |childrens| {
        defer allocator.free(childrens);

        const parent_type_str = public.EntityCdb.readStr(top_level_obj_r, .Storage) orelse "";
...
pub fn createWorld() !*public.World {
    ...
    {
        var zz = profiler.ZoneN(@src(), "ECS: import flecs");
        defer zz.End();
    ...
    // Register components
    {
        var zz = profiler.ZoneN(@src(), "ECS: register components");
        defer zz.End();

        const components_impl = apidb.getImpl(_allocator, public.ComponentI) catch undefined;
        ...
        for (components_impl) |iface| {
            ...
            const instansiate_trait_pair: ?zflecs.id_t = switch (iface.on_instantiate) {
                .DontInherit => zflecs.make_pair(EcsOnInstantiate, EcsDontInherit),
                ...
                _ = zflecs.component_init(world, &.{
                    ...
                    .type = .{
                        ...
                        .hooks = .{
                            ...
                            .ctx = if (iface.create_manager) |createManager| try createManager(wd) else null,

                            .ctor = if (iface.on_create) |onCreate| @ptrCast(onCreate) else null,
                            .dtor = if (iface.on_destroy) |onDestroy| @ptrCast(onDestroy) else null,
                            .copy = if (iface.on_copy) |onCopy| @ptrCast(onCopy) else null,
                            .move = if (iface.on_move) |onMove| @ptrCast(onMove) else null,

                            .on_add = if (iface.on_add != null) struct {
                                pub fn f(iter: *zflecs.iter_t) callconv(.c) void {
                                    ...
                                    component_i.?.on_add.?(manager, &it) catch undefined;
                                }
                            }.f else null,

                            .on_set = if (iface.on_set != null) struct {
                                pub fn f(iter: *zflecs.iter_t) callconv(.c) void {
                                    const manager: ?*anyopaque = @ptrCast(@alignCast(iter.ctx));
                                    const component_i: ?*public.ComponentI = @ptrCast(@alignCast(iter.callback_ctx));
                                    ...
                                    component_i.?.on_set.?(manager, &it) catch undefined;
                                }
                            }.f else null,

                            .on_remove = if (iface.on_remove != null) struct {
                                pub fn f(iter: *zflecs.iter_t) callconv(.c) void {
                                    const manager: ?*anyopaque = @ptrCast(@alignCast(iter.ctx));
                                    const component_i: ?*public.ComponentI = @ptrCast(@alignCast(iter.callback_ctx));
                                    ...
                                    component_i.?.on_remove.?(manager, &it) catch undefined;
                                }
                            }.f else null,
                        },
                    },
    ...
    // Register systems
    {
        var zz = profiler.ZoneN(@src(), "ECS: register systems");
        defer zz.End();

        const systems_impl = apidb.getImpl(_allocator, public.SystemI) catch undefined;
        ...
        for (systems_impl) |iface| {
            ...
            if (iface.order_by_component) |c| {
                system_desc.query.order_by = w.id_map.find(c).?;
                system_desc.query.order_by_callback = iface.order_by_callback;
            }
    ...
    // Register Observers
    {
        var zz = profiler.ZoneN(@src(), "ECS: register observers");
        defer zz.End();

        const observers_iface = apidb.getImpl(_allocator, public.ObserverI) catch undefined;
...
fn api_task_new(clb: zflecs.os.thread_callback_t, data: ?*anyopaque) callconv(.c) zflecs.os.thread_t {
    const Task = struct {
        ...
        data: ?*anyopaque,
        pub fn exec(s: *@This()) !void {
            var zone = profiler.ZoneN(@src(), "ECS workload");
            defer zone.End();
            _ = s.clb(s.data);
        }
    };

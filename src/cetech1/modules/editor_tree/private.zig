...
pub const c = @cImport({
    @cInclude("cetech1/modules/editor_tree/editor_tree.h");
});
...
fn cdbTreeNode(
    ...
    _ = args;
    return _editorui.treeNodeFlags(label, .{ .open_on_arrow = true, .open_on_double_click = true, .default_open = default_open, .no_tree_push_on_open = no_push, .selected = selected, .leaf = leaf });
}
...
fn cdbTreeView(allocator: std.mem.Allocator, db: *cetech1.cdb.CdbDb, obj: cetech1.cdb.ObjId, selection: cetech1.cdb.ObjId, args: editor.CdbTreeViewArgs) !void {
    // if exist aspect use it
    const ui_aspect = db.getAspect(editor.UiTreeAspect, obj.type_hash);
    ...
    for (prop_defs, 0..) |prop_def, idx| {
        ...
        switch (prop_def.type) {
            ...
            .SUBOBJECT => {
                ...
                if (db.getAspect(editor.UiEmbedPropertiesAspect, subobj.type_hash) != null) {
                    continue;
                }
                ...
                const open = api.cdbTreeNode(label, false, false, _editor.isSelected(db, selection, subobj), isLeaf(db, subobj), args);
                ...
                if (_editorui.isItemActivated()) {
                    try _editor.handleSelection(allocator, db, selection, subobj, args.multiselect);
                }
            ...
            .SUBOBJECT_SET => {
                ...
                if (_editorui.beginPopupContextItem()) {
                    try _editor.objContextMenu(allocator, db, obj, prop_idx, null);
                ...
                if (open) {
                    ...
                    if (set) |s| {
                        ...
                        for (s, 0..) |subobj, set_idx| {
                            ...
                            const label = _editor.buffFormatObjLabel(&buff, allocator, db, subobj) orelse try std.fmt.bufPrintZ(&buff, "{d}", .{set_idx});
                            ...
                            const open_inset = api.cdbTreeNode(label, true, false, _editor.isSelected(db, selection, subobj), isLeaf(db, subobj), args);
                            ...
                            if (_editorui.beginPopupContextItem()) {
                                defer _editorui.endPopup();
                                try _editor.objContextMenu(allocator, db, obj, prop_idx, subobj);
                            }
                    ...
                    if (set) |s| {
                        ...
                        for (s, 0..) |subobj, set_idx| {
                            ...
                            if (_assetdb.getUuid(subobj)) |uuid| {
                                label = try std.fmt.bufPrintZ(&buff, editor.Icons.Deleted ++ " " ++ "{s}###{}", .{ uuid, uuid });
                            } else {
                                label = try std.fmt.bufPrintZ(&buff, editor.Icons.Deleted ++ " " ++ "{d}:{d}###{d}{d}", .{ subobj.id, subobj.type_hash.id, subobj.id, subobj.type_hash.id });
                            }

                            _editorui.pushStyleColor4f(.{ .idx = .text, .c = REMOVED_COLOR });
                            const open_inset = _editorui.treeNodeFlags(label.?, .{ .leaf = true, .selected = _editor.isSelected(db, selection, subobj) });
                            _editorui.popStyleColor(.{});
                            if (open_inset) {
                                if (_editorui.isItemActivated()) {
                                    try _editor.handleSelection(allocator, db, selection, subobj, args.multiselect);
                                ...
                                if (_editorui.beginPopupContextItem()) {
                                    defer _editorui.endPopup();
                                    if (_editorui.menuItem(editor.Icons.Revive ++ "  " ++ "Restore deleted", .{})) {
...
pub fn load_module_zig(apidb: *cetech1.apidb.ApiDbAPI, allocator: Allocator, log: *cetech1.log.LogAPI, load: bool, reload: bool) anyerror!bool {
    ...
    try apidb.setOrRemoveZigApi(editor.TreeAPI, &api, load);
...
comptime {
    std.debug.assert(@sizeOf(c.ct_editorui_ui_tree_aspect) == @sizeOf(editor.UiTreeAspect));
    std.debug.assert(@sizeOf(c.ct_editor_cdb_tree_args) == @sizeOf(editor.CdbTreeViewArgs));
}

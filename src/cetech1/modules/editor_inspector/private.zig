...
pub const c = @cImport({
    @cInclude("cetech1/modules/editor_properties/editor_properties.h");
});
...
const G = struct {
    ...
    color4f_properties_aspec: *editor.UiEmbedPropertiesAspect = undefined,
    folder_name_prop_aspect: *editor.UiPropertyAspect = undefined,
    folder_property_config_aspect: *editor.UiPropertiesConfigAspect = undefined,
};
var _g: *G = undefined;

var api = editor.InspectorAPI{
    ...
    .openNewInspectorForObj = openNewInspectorForObj,
};
...
fn onSetProperty(selected_data: ?*anyopaque, data: editor.UiModalI.Data) callconv(.C) void {
    ...
    var w = _g.main_db.writeObj(params.obj).?;
...
fn onSetNewPrototype(selected_data: ?*anyopaque, data: editor.UiModalI.Data) callconv(.C) void {
    var params = std.mem.bytesToValue(editor.SelectAssetParams, data.data[0..@sizeOf(editor.SelectAssetParams)]);

    var selection: *cetech1.cdb.ObjId = @alignCast(@ptrCast(selected_data orelse return));

    var tmp_arena = _tempalloc.createTempArena() catch undefined;
    defer _tempalloc.destroyTempArena(tmp_arena);
    const allocator = tmp_arena.allocator();

    var selected_obj = _editor.getFirstSelected(allocator, &_g.main_db, selection.*);
...
fn onAddToSet(selected_data: ?*anyopaque, data: editor.UiModalI.Data) callconv(.C) void {
    var selection: *cetech1.cdb.ObjId = @alignCast(@ptrCast(selected_data orelse return));
    ...
    if (_editor.getSelected(allocator, &_g.main_db, selection.*)) |selected| {
        defer allocator.free((selected));
        var w = _g.main_db.writeObj(params.obj).?;
        defer _g.main_db.writeCommit(w);

        for (selected) |asset| {
            var asset_obj = cetech1.assetdb.AssetType.readSubObj(&_g.main_db, _g.main_db.readObj(asset).?, .Object).?;
            ...
            if (prop_type == .REFERENCE_SET) {
                ...
                var sub_w = _g.main_db.writeObj(asset_obj).?;
...
fn uiAssetInput(
    ...
    var defs = db.getTypePropDef(obj.type_hash).?;
    var prop_def = defs[prop_idx];
    ...
    var data = editor.UiModalI.makeData(editor.SelectAssetParams{
        .obj = obj,
        .prop_idx = prop_idx,
        .ignored_object = obj,
...
fn uiAssetAddToSetInput(
    ...
    if (_editorui.button(editor.Icons.Add, .{})) {
        var data = editor.UiModalI.makeData(editor.SelectAssetParams{ .obj = obj, .prop_idx = prop_idx, .ignored_object = obj, .only_types = prop_def.type_hash, .multiselect = true });
        _editor.openModal(editor.SelectAssetModal, onAddToSet, data);
    }
}
...
fn uiAssetInputGeneric(
    allocator: std.mem.Allocator,
    db: *cetech1.cdb.CdbDb,
    on_set: editor.UiModalI.OnSetFN,
    ...
    var value_asset: ?cetech1.cdb.ObjId = _assetdb.getAssetForObj(value_obj);

    if (value_asset) |asset| {
        var path = try _assetdb.getFilePathForAsset(asset, allocator);
        defer allocator.free(path);
        asset_name = try std.fmt.bufPrintZ(&buff, "{s}", .{path});
    } else {
        if (cetech1.assetdb.FolderType.isSameType(value_obj)) {
            var path = try _assetdb.getPathForFolder(value_obj, allocator);
            defer allocator.free(path);
    ...
    if (_editorui.beginPopup("ui_asset_context_menu", .{})) {
        if (_editorui.menuItem("Select asset", .{ .enabled = !read_only })) {
            _editor.openModal(editor.SelectAssetModal, on_set, data);
        }

        if (_editorui.menuItem("Open new inspector", .{ .enabled = value_asset != null })) {
            api.openNewInspectorForObj(db, value_asset.?);
        }
...
fn beginSection(label: [:0]const u8, leaf: bool, default_open: bool) bool {
    var open = _editorui.treeNodeFlags(label, .{
        .framed = true,
        .leaf = leaf,
...
fn cdbPropertiesView(allocator: std.mem.Allocator, db: *cetech1.cdb.CdbDb, obj: cetech1.cdb.ObjId, args: editor.cdbPropertiesViewArgs) !void {
...
fn objContextMenuBtn(allocator: std.mem.Allocator, db: *cetech1.cdb.CdbDb, obj: cetech1.cdb.ObjId, prop_idx: ?u32, in_set_obj: ?cetech1.cdb.ObjId) !void {
    if (_editorui.beginPopup("property_obj_menu", .{})) {
        try _editor.objContextMenu(allocator, db, obj, prop_idx, in_set_obj);
        _editorui.endPopup();
    }
...
fn cdbPropertiesObj(
    ...
    var ui_aspect = db.getAspect(editor.UiPropertiesAspect, obj.type_hash);
    if (ui_aspect) |aspect| {
    ...
    var config_aspect = db.getAspect(editor.UiPropertiesConfigAspect, obj.type_hash);
    ...
    if (beginPropTable("prop2")) {
        ...
        for (prop_defs, 0..) |prop_def, idx| {
            ...
            var ui_prop_aspect: ?*editor.UiPropertyAspect = null;
            if (prop_def.type != .REFERENCE_SET and prop_def.type != .SUBOBJECT_SET) {
                ui_prop_aspect = db.getPropertyAspect(editor.UiPropertyAspect, obj.type_hash, prop_idx);
            ...
            switch (prop_def.type) {
                .REFERENCE_SET, .SUBOBJECT_SET => {
                    var ui_embed_prop_aspect = db.getPropertyAspect(editor.UiEmbedPropertyAspect, obj.type_hash, prop_idx);
                    if (ui_embed_prop_aspect) |aspect| {
                        var lbl = try std.fmt.bufPrintZ(&buff, "{s}", .{prop_name});
                ...
                .SUBOBJECT => {
                    var subobj = db.readSubObj(obj_r, prop_idx);
                    var ui_embed_prop_aspect = db.getAspect(editor.UiEmbedPropertiesAspect, prop_def.type_hash);
                    var lbl = try std.fmt.bufPrintZ(&buff, "{s}", .{prop_name});
                    if (ui_embed_prop_aspect) |aspect| {
                        if (uiPropLabel(allocator, lbl, prop_color, args)) {
                            _editorui.tableNextColumn();
    ...
    for (prop_defs, 0..) |prop_def, idx| {
        ...
        var ui_prop_aspect = db.getPropertyAspect(editor.UiPropertyAspect, obj.type_hash, prop_idx);
        // If exist aspect and is empty hide property.
        if (ui_prop_aspect) |aspect| {
            if (aspect.ui_property == null) continue;
        ...
        switch (prop_def.type) {
            .SUBOBJECT => {
                ...
                if (db.getAspect(editor.UiEmbedPropertiesAspect, prop_def.type_hash) != null) continue;
                ...
                var open = beginSection(label, subobj == null, true);
            ...
            .SUBOBJECT_SET, .REFERENCE_SET => {
                if (db.getPropertyAspect(editor.UiEmbedPropertyAspect, obj.type_hash, prop_idx) != null) {
                    continue;
                }
                ...
                var open = beginSection(prop_label, false, true);
                defer endSection(open);
                ...
                if (open) {
                    ...
                    if (set) |s| {
                        ...
                        for (s, 0..) |subobj, set_idx| {
                            var label = _editor.buffFormatObjLabel(&buff, allocator, db, subobj) orelse try std.fmt.bufPrintZ(&buff, "{d}", .{set_idx});
                            ...
                            var open_inset = beginSection(label, false, true);
...
fn uiInputForProperty(db: *cetech1.cdb.CdbDb, obj: cetech1.cdb.ObjId, prop_idx: u32) !void {
    ...
    var reader = db.readObj(obj) orelse return;
...
fn uiPropLabel(allocator: std.mem.Allocator, name: [:0]const u8, color: ?[4]f32, args: editor.cdbPropertiesViewArgs) bool {
    if (args.filter) |filter| {
...
fn assetUiProperiesAspect(
    ...
    if (beginPropTable("prop2")) {
        ...
        // Tags
        // TODO: SHIT HACK
        var ui_prop_aspect = db.getPropertyAspect(editor.UiEmbedPropertyAspect, obj.type_hash, cetech1.assetdb.AssetType.propIdx(.Tags));
        // If exist aspect and is empty hide property.
        if (ui_prop_aspect) |aspect| {
...
// Asset properties aspect
var color4f_properties_aspec = editor.UiEmbedPropertiesAspect.implement(color4fUiProperiesAspect);
...
fn tabUi(inst: *editor.TabO) void {
    ...
    if (_editorui.beginChild("Inspector", .{ .border = false })) {
        ...
        const selectected_count = _editor.selectedCount(allocator, &tab_o.db, tab_o.selected_obj);
        if (selectected_count == 0) return;

        var obj: cetech1.cdb.ObjId = tab_o.selected_obj;

        if (selectected_count == 1) {
            obj = _editor.getFirstSelected(allocator, &tab_o.db, tab_o.selected_obj);
        }
...
fn cdbCreateTypes(db_: *cetech1.cdb.Db) !void {
    ...
    try cetech1.cdb_types.Color4fType.addAspect(
        &db,
        editor.UiEmbedPropertiesAspect,
...
pub fn load_module_zig(apidb: *cetech1.apidb.ApiDbAPI, allocator: Allocator, log: *cetech1.log.LogAPI, load: bool, reload: bool) anyerror!bool {
    ...
    _g.color4f_properties_aspec = try apidb.globalVar(editor.UiEmbedPropertiesAspect, MODULE_NAME, COLOR4F_PROPERTY_ASPECT_NAME, .{});
    _g.color4f_properties_aspec.* = color4f_properties_aspec;

    _g.folder_property_config_aspect = try apidb.globalVar(editor.UiPropertiesConfigAspect, MODULE_NAME, FOLDER_PROPERTY_CONFIG_ASPECT_NAME, .{});
    ...
    try apidb.setOrRemoveZigApi(editor.InspectorAPI, &api, load);
...
comptime {
    ...
    std.debug.assert(@sizeOf(c.ct_editorui_ui_property_aspect) == @sizeOf(editor.UiPropertyAspect));
    std.debug.assert(@sizeOf(c.ct_editor_cdb_proprties_args) == @sizeOf(editor.cdbPropertiesViewArgs));
}

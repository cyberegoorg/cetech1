...
fn renderAllViewports(allocator: std.mem.Allocator) !void {
    ...
    for (_viewport_set.keys()) |viewport| {
        ...
        if (recreate) {
            ...
            const txFlags: u64 = 0 | gfx.TextureFlags_Rt | gfx.SamplerFlags_MinPoint | gfx.SamplerFlags_MagPoint | gfx.SamplerFlags_MipMask | gfx.SamplerFlags_MagPoint | gfx.SamplerFlags_MipPoint | gfx.SamplerFlags_UClamp | gfx.SamplerFlags_VClamp | gfx.SamplerFlags_MipPoint;
        ...
        const Task = struct {
            ...
            pub fn exec(s: *@This()) !void {
                ...
                const builder = try rg.createBuilder();
                defer rg.destroyBuilder(builder);

                try rg.setupBuilder(builder);
                try builder.render(
                    &gfx_api,
                    .{ .ptr = s.viewport, .vtable = &viewport_vt },
                );
            }
        };
...
fn _renderAll(ctx: *GpuContext, kernel_tick: u64, dt: f32, vsync: bool) !void {
    ...
    // TODO: save for sync (sync across frames like read back frame + 2)
    {

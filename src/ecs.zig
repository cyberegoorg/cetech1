...
const World = struct {
    ...
    pub fn setId(self: *World, entity: public.EntityId, id: cetech1.StrId32, size: usize, ptr: ?*const anyopaque) public.EntityId {
        return zflecs.set_id(self.w, entity, self.id_map.find(id).?, size, ptr);
    }

    pub fn getMutId(self: *World, entity: public.EntityId, id: cetech1.StrId32) ?*anyopaque {
...
pub fn progressAll(allocator: std.mem.Allocator, dt: f32) !void {
    ...
    if (parallel) {
        var tasks = try cetech1.ArrayList(cetech1.task.TaskID).initCapacity(allocator, _world_data.count());

...
fn _loadDynLib(path: []const u8) !DynLibInfo {
    var dll = std.DynLib.open(path) catch |err| {
        log.err("Error load module from {s} with error {any}\n", .{ path, err });
        return err;
    };

    var load_fce_name_buff: [128:0]u8 = undefined;
    const module_name = getModuleName(path);
    const load_fce_name = try std.fmt.bufPrintZ(&load_fce_name_buff, "ct_load_module_{s}", .{module_name});

    const symbol = dll.lookup(*c.ct_module_fce_t, load_fce_name);
    if (symbol == null) {
        log.err("Error find symbol {s} in {s}\n", .{ load_fce_name, path });
    ...
    return .{
        .name = try _allocator.dupeZ(u8, module_name),
...
pub fn loadDynModules() !void {
    ...
    while (try iterator.next()) |path| {
        ...
        log.debug("Loading module from {s}", .{full_path});
    ...
    if (modules_reloaded) {
        ...
        for (to_reload.items) |k| {
            ...
            if (0 == old_module_desc.desc.module_fce.?(&apidb.apidb_global_c, @ptrCast(&alloc_item.*.allocator), 0, 1)) {
                log.err("Problem with unload old module {s}\n", .{k});
                continue;
            }
            ...
            if (0 == new_dyn_lib_info.symbol(&apidb.apidb_global_c, @ptrCast(&alloc_item.*.allocator), 1, 1)) {
                log.err("Problem with load new module {s}\n", .{k});
                continue;
            }

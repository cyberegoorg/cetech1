...
const log = std.log.scoped(.apidb);
...
fn setApiOpaqueue(language: []const u8, api_name: []const u8, api_ptr: *const anyopaque, api_size: usize) !void {
...
fn implInterface(interface_name: strid.StrId64, impl_ptr: *const anyopaque) anyerror!void {
...
fn removeImpl(interface_name: strid.StrId64, impl_ptr: *const anyopaque) void {
    var impl_list = _interafce_map.getPtr(interface_name);
...
pub fn dumpInterfaces() void {
    // TODO
    // log.debug("IMPLEMENTED INTERAFCES", .{});
...
pub const apidb_global_c = blk: {
    const c_api = struct {
        ...
        pub fn set_api(language: [*c]const u8, api_name: [*c]const u8, api_ptr: ?*const anyopaque, api_size: u32) callconv(.C) void {
        ...
        pub fn set_or_remove(language: [*c]const u8, api_name: [*c]const u8, api_ptr: ?*const anyopaque, api_size: u32, load: bool, reload: bool) callconv(.C) void {
            if (load) {
        ...
        pub fn impl_or_remove(interface_name: c.c.ct_strid64_t, api_ptr: ?*const anyopaque, load: bool) callconv(.C) void {
            if (load) {
        ...
        pub fn impl(interface_name: c.c.ct_strid64_t, api_ptr: ?*const anyopaque) callconv(.C) void {
            return implInterface(strid.StrId64.from(c.c.ct_strid64_t, interface_name), api_ptr.?) catch return;
        }
        pub fn remove_impl(interface_name: c.c.ct_strid64_t, api_ptr: ?*const anyopaque) callconv(.C) void {

...
pub fn JobSystem(comptime queue_config: QueueConfig) type {
    ...
    const Slot = struct {
        ...
        fn storeJob(
...

            {
                ...
                // const c = self.cycle.fetchAdd(1, .monotonic);
                // std.debug.assert(c == old_cycle);
    ...
    const Queue = struct {
        ...
        pub fn schedule(self: *Self, prereq: public.TaskID, job: anytype, combinee: bool) !public.TaskID {
        ...
        fn getWorkToDo(self: *Self) ?public.TaskID {
            ...
            {
                defer {
                    for (0..self_w.buffer.items.len) |value| {
                        self_w.pushJob(self_w.buffer.items[self_w.buffer.items.len - value - 1]);
                    }
                    self_w.buffer.clearRetainingCapacity();
                }

                while (self_w.popJob()) |task| {
                    ...
                    // TODO: SHIT
                    if (slot.combine) {
                        const d: *CombinePrereqsJob = @alignCast(@ptrCast(&slot.data));
                        var done = true;
                        for (d.prereqs) |p| {
                            if (!self.isDone(p)) {
                                done = false;
        ...
        const CombinePrereqsJob = struct {
            ...
            jobs: *Self,
            prereqs: [max_prereqs]public.TaskID = [_]public.TaskID{public.TaskID.none} ** max_prereqs,
            ...
            pub fn exec(self: *@This()) void {
                ...
                for (self.prereqs) |prereq| {
                    if (true) {
                        self.jobs.waitForManyTask(&.{prereq});
                    } else {
                        while (!self.jobs.isDone(prereq)) {
                            std.time.sleep(queue_config.idle_sleep_ns);
                        }
                    }
                }
        ...
        pub fn combine(self: *Self, prereqs: []const public.TaskID) !public.TaskID {
            ...
            while (in.len > 0) {
                ...
                id = try self.schedule(id, job, true);
            }
            return id;
        }
    };

...
const CullingSystem = struct {
    ...
    pub fn doCulling(self: *Self, viewers: []gfx_rg.Viewer) !void {
        ...
        // for (self.crq_map.keys(), self.crq_map.values()) |k, value| {
        //     var result = self.getResult(k);
        //     try result.appendMany(value.mtx.items, value.renderables.items);
        // }
    }
};
...
const render_component_c = ecs.ComponentI.implement(public.RenderComponent, struct {
    pub fn onDestroy(components: []public.RenderComponent) !void {
        for (components) |c| {
    ...
    pub fn onMove(dsts: []public.RenderComponent, srcs: []public.RenderComponent) !void {
    ...
    pub fn onSet(iter: *ecs.IterO) !void {
        var it = ecs_private.api.toIter(iter);

        const components = it.field(public.RenderComponent, 1).?;
        for (components) |component| {
            // TODO: real multi call
            try graph_private.api.executeNode(&.{component.graph_container}, graphvm.EVENT_INIT_NODE_TYPE);
        }
    }
    ...
    pub fn onRemove(iter: *ecs.IterO) !void {
        ...
        for (components) |component| {
            // TODO: real multi call
            try graph_private.api.executeNode(&.{component.graph_container}, graphvm.EVENT_SHUTDOWN_NODE_TYPE);
        }
    }
});
...
const remder_component_renderer_i = cetech1.renderer.ComponentRendererI.implement(public.RenderComponent, struct {
    pub fn culling(allocator: std.mem.Allocator, builder: gfx_rg.GraphBuilder, world: ecs.World, viewers: []gfx_rg.Viewer, rq: *public.CullingRequest) !void {
        ...
        while (q.next(it)) {
            const transforms = it.field(transform.WorldTransform, 1).?;
            const render_components = it.field(public.RenderComponent, 2).?;

            const containers = toContanerSlice(render_components);
            try graph_private.api.executeNode(containers, graphvm.CULLING_VOLUME_NODE_TYPE);
            ...
            for (states) |volume| {
                ...
                try renderables.append(culling_volume.*);
            }
        ...
        if (gpu_private.gfx_api.getEncoder()) |e| {
            const dd = viewport.getDD();
            {
                ...
                if (culling_result) |result| {
                    ...
                    //log.debug("renderables {}", .{volumes.len});
...
pub fn registerToApi() !void {
    ...
    try apidb.api.implOrRemove(module_name, cetech1.renderer.ComponentRendererI, &remder_component_renderer_i, true);
}

fn createViewport(render_graph: gfx_rg.RenderGraph, world: ?cetech1.ecs.World) !public.Viewport {
    ...
    pub fn exec(s: *@This()) !void {
        ...
        if (s.viewport.world) |world| {
            ...
            // Collect  renderables to culling
            {
                ...
                var it = apidb.api.getFirstImpl(cetech1.renderer.ComponentRendererI);
                while (it) |node| : (it = node.next) {
                    const iface = cetech1.apidb.ApiDbAPI.toInterface(cetech1.renderer.ComponentRendererI, node);

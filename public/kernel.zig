...
pub const KernelTaskUpdateI = extern struct {
    ...
    depends: [*]const strid.StrId64,
    depends_n: u64,
    update: *const fn (allocator: *const std.mem.Allocator, kernel_tick: u64, dt: f32) callconv(.C) void,

    pub inline fn implment(
        phase: strid.StrId64,
        name: [:0]const u8,
        depends: []const strid.StrId64,
        update: *const fn (allocator: std.mem.Allocator, kernel_tick: u64, dt: f32) anyerror!void,
    ) KernelTaskUpdateI {
        return KernelTaskUpdateI{
            ...
            .depends_n = depends.len,
            .update = struct {
                pub fn f(allocator: *const std.mem.Allocator, kernel_tick: u64, dt: f32) callconv(.C) void {
                    update(allocator.*, kernel_tick, dt) catch undefined;
                }
            }.f,
        };
    }

...
pub fn CdbTypeDecl(comptime type_name: [:0]const u8, comptime props_enum: type, comptime extend: type) type {
    return struct {
        ...
        pub fn isSameType(db: *CdbDb, obj: ObjId) bool {
            return obj.type_idx.eql(db.getTypeIdx(Self.type_hash).?);
        }

        pub fn typeIdx(db: *CdbDb) TypeIdx {
        ...
        pub fn destroyObject(db: *CdbDb, obj: ObjId) void {
...
pub const CdbDb = struct {
    ...
    pub fn getTypeName(self: *Self, type_idx: TypeIdx) ?[]const u8 {
        return self.cdbapi.getTypeNameFn(self.db, type_idx);
    }

    /// Get type definition for type hash.
    pub fn getTypePropDef(self: *Self, type_idx: TypeIdx) ?[]const PropDef {
        return self.cdbapi.getTypePropDefFn(self.db, type_idx);
    }
    ...
    pub fn stressIt(self: *Self, type_idx: TypeIdx, type_hash2: TypeIdx, ref_obj1: ObjId) anyerror!void {
        try self.cdbapi.stressItFn(self.db, type_idx, type_hash2, ref_obj1);
    }
    ...
    pub fn getDefaultObject(self: *Self, type_idx: TypeIdx) ?ObjId {
        return self.cdbapi.getDefaultObjectFn(self.db, type_idx);
    }

    pub fn getFirstObject(self: *Self, type_idx: TypeIdx) ?ObjId {
        return self.cdbapi.getFirstObjectFn(self.db, type_idx);
    }

    pub fn getAllObjectByType(self: *Self, allocator: std.mem.Allocator, type_idx: TypeIdx) ?[]ObjId {
        return self.cdbapi.getAllObjectByTypeFn(self.db, allocator, type_idx);
    }

    pub fn hasTypeSet(self: *Self, type_idx: TypeIdx) bool {
        return self.cdbapi.hasTypeSetFn(self.db, type_idx);
    }

    pub fn hasTypeSubobject(self: *Self, type_idx: TypeIdx) bool {
        return self.cdbapi.hasTypeSubobjectFn(self.db, type_idx);
    }

    pub fn getTypeIdx(self: *Self, type_hash: strid.StrId32) ?TypeIdx {
        return self.cdbapi.getTypeIdxFn(self.db, type_hash);
    }
...
pub const CdbAPI = struct {
    ...
    // Type operation
    addTypeFn: *const fn (db: *Db, name: []const u8, prop_def: []const PropDef) anyerror!TypeIdx,
    getTypeNameFn: *const fn (db: *Db, type_idx: TypeIdx) ?[]const u8,
    getTypePropDefFn: *const fn (db: *Db, type_idx: TypeIdx) ?[]const PropDef,
    getTypePropDefIdxFn: *const fn (db: *Db, type_idx: TypeIdx, prop_name: []const u8) ?u32,

    // Aspects
    addAspectFn: *const fn (db: *Db, type_idx: TypeIdx, apect_name: []const u8, aspect_ptr: *anyopaque) anyerror!void,
    getAspectFn: *const fn (db: *Db, type_idx: TypeIdx, aspect_hash: strid.StrId32) ?*anyopaque,

    addPropertyAspectFn: *const fn (db: *Db, type_idx: TypeIdx, prop_idx: u32, apect_name: []const u8, aspect_ptr: *anyopaque) anyerror!void,
    getPropertyAspectFn: *const fn (db: *Db, type_idx: TypeIdx, prop_idx: u32, aspect_hash: strid.StrId32) ?*anyopaque,
    ...
    // Object operation
    createObjectFn: *const fn (db: *Db, type_idx: TypeIdx) anyerror!ObjId,
    ...
    getDefaultObjectFn: *const fn (db: *Db, type_idx: TypeIdx) ?ObjId,
    setPrototypeFn: *const fn (db: *Db, obj: ObjId, prototype: ObjId) anyerror!void,
    getFirstObjectFn: *const fn (db_: *Db, type_idx: TypeIdx) ObjId,
    getAllObjectByTypeFn: *const fn (db_: *Db, tmp_allocator: std.mem.Allocator, type_idx: TypeIdx) ?[]ObjId,
    ...
    stressItFn: *const fn (db: *Db, type_idx: TypeIdx, type_hash2: TypeIdx, ref_obj1: ObjId) anyerror!void,

    gcFn: *const fn (db: *Db, tmp_allocator: std.mem.Allocator) anyerror!void,
    dump: *const fn (db: *Db) anyerror!void,

    hasTypeSetFn: *const fn (db: *Db, type_idx: TypeIdx) bool,
    hasTypeSubobjectFn: *const fn (db: *Db, type_idx: TypeIdx) bool,

    getTypeIdxFn: *const fn (db: *Db, type_hash: strid.StrId32) ?TypeIdx,
    //#endregion
};

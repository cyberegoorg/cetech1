...
pub const NodeType = cdb.CdbTypeDecl(
    ...
    struct {
        pub fn getNodeTypeId(db: cdb.Db, reader: *cdb.Obj) strid.StrId32 {
            ...
            //return .{ .id = NodeType.readValue(db, u32, reader, .node_type) };
        }
    },
);
...
pub const GraphVMApi = struct {
    ...
    createInstances: *const fn (allocator: std.mem.Allocator, db: cdb.Db, graph: cdb.ObjId, count: usize) anyerror![]GraphInstance,
    ...
    buildInstances: *const fn (instances: []const GraphInstance) anyerror!void,

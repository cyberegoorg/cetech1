...
pub const FrustumPlanes = struct {
    ...
    pub fn vsSphereNaive(self: FrustumPlanes, center: Vec3f, radius: f32) bool {
        for (0..6) |idx| {
            const world_space_point = center.toF32x4();
            const dot = zm.dot3(world_space_point, self.p[idx].v);
            const dist = dot[0] + self.p[idx].v[3] + radius;
...
pub fn calcBoundingSphereForCone(origin: Vec3f, forward: Vec3f, size: f32, angle: f32) F32x4 {
    var center: Vec3f = .{};
    var radius: f32 = 0;
    ...
    if (angle > std.math.pi / 4.0) {
        ...
        center = .mulAdd(forward, .splat(cos_angle * size), origin);
    } else {
        radius = size / (2.0 * cos_angle);
        center = .mulAdd(forward, .splat(radius), origin);
    }

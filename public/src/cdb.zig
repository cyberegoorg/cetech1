...
pub fn CdbTypeDecl(comptime type_name: [:0]const u8, comptime props_enum: type, comptime extend: type) type {
    return struct {
        ...
        pub fn isSameType(db: Db, obj: ObjId) bool {
            return obj.type_idx.eql(db.getTypeIdx(Self.type_hash).?);
        }

        pub fn typeIdx(db: Db) TypeIdx {
        ...
        pub fn addAspect(db: Db, comptime T: type, aspect_ptr: *T) !void {
            try db.addAspect(T, db.getTypeIdx(Self.type_hash).?, aspect_ptr);
        }

        pub fn getAspect(db: Db, comptime T: type) ?*T {
            return db.getAspect(T, db.getTypeIdx(Self.type_hash).?);
        }

        pub fn addPropertyAspect(db: Db, comptime T: type, prop: PropsEnum, aspect_ptr: *T) !void {
            try db.addPropertyAspect(T, db.getTypeIdx(Self.type_hash).?, Self.propIdx(prop), aspect_ptr);
        }

        pub fn getPropertyAspect(db: Db, comptime T: type, prop: PropsEnum) ?*T {
            try db.getPropertyAspect(T, db.getTypeIdx(Self.type_hash).?, Self.propIdx(prop));
        }

        pub fn createObject(db: Db) !ObjId {
            return db.createObject(db.getTypeIdx(Self.type_hash).?);
        }

        pub fn destroyObject(db: Db, obj: ObjId) void {
            return db.destroyObject(obj);
        }

        pub fn read(db: Db, obj: ObjId) ?*Obj {
            return db.readObj(obj);
        }

        pub fn write(db: Db, obj: ObjId) ?*Obj {
            return db.writeObj(obj);
        }

        pub fn commit(db: Db, writer: *Obj) !void {
            return db.writeCommit(writer);
        }

        pub fn readValue(db: Db, comptime T: type, reader: *Obj, prop: PropsEnum) T {
            return db.readValue(T, reader, Self.propIdx(prop));
        }

        pub fn setValue(db: Db, comptime T: type, writer: *Obj, prop: PropsEnum, value: T) void {
            db.setValue(T, writer, Self.propIdx(prop), value);
        }

        pub fn setStr(db: Db, writer: *Obj, prop: PropsEnum, value: [:0]const u8) !void {
            return db.setStr(writer, Self.propIdx(prop), value);
        }

        pub fn readStr(db: Db, reader: *Obj, prop: PropsEnum) ?[:0]const u8 {
            return db.readStr(reader, Self.propIdx(prop));
        }
        pub fn setSubObj(db: Db, writer: *Obj, prop: PropsEnum, subobj_writer: *Obj) !void {
            try db.setSubObj(writer, Self.propIdx(prop), subobj_writer);
        }

        pub fn readSubObj(db: Db, reader: *Obj, prop: PropsEnum) ?ObjId {
            return db.readSubObj(reader, Self.propIdx(prop));
        }

        pub fn setRef(db: Db, writer: *Obj, prop: PropsEnum, value: ObjId) !void {
            return db.setRef(writer, Self.propIdx(prop), value);
        }

        pub fn readRef(db: Db, reader: *Obj, prop: PropsEnum) ?ObjId {
            return db.readRef(reader, Self.propIdx(prop));
        }

        pub fn readRefSet(db: Db, reader: *Obj, prop: PropsEnum, allocator: std.mem.Allocator) ?[]const ObjId {
            return db.readRefSet(reader, Self.propIdx(prop), allocator);
        }

        pub fn readRefSetAdded(db: Db, reader: *Obj, prop: PropsEnum, allocator: std.mem.Allocator) ?[]const ObjId {
            return db.readRefSetAdded(reader, Self.propIdx(prop), allocator);
        }

        pub fn readRefSetRemoved(db: Db, reader: *Obj, prop: PropsEnum, allocator: std.mem.Allocator) ?[]const ObjId {
            return db.readRefSetRemoved(reader, Self.propIdx(prop), allocator);
        }

        pub fn readSubObjSetAdded(db: Db, reader: *Obj, prop: PropsEnum, allocator: std.mem.Allocator) !?[]const ObjId {
            return db.readSubObjSetAdded(reader, Self.propIdx(prop), allocator);
        }

        pub fn readSubObjSetRemoved(db: Db, reader: *Obj, prop: PropsEnum, allocator: std.mem.Allocator) ?[]const ObjId {
            return db.readSubObjSetRemoved(reader, Self.propIdx(prop), allocator);
        }

        pub fn addRefToSet(db: Db, writer: *Obj, prop: PropsEnum, values: []const ObjId) !void {
            try db.addRefToSet(writer, Self.propIdx(prop), values);
        }

        pub fn removeFromRefSet(db: Db, writer: *Obj, prop: PropsEnum, value: ObjId) !void {
            try db.removeFromRefSet(writer, Self.propIdx(prop), value);
        }

        pub fn addSubObjToSet(db: Db, writer: *Obj, prop: PropsEnum, subobj_writers: []const *Obj) !void {
            try db.addSubObjToSet(writer, Self.propIdx(prop), subobj_writers);
        }

        pub fn readSubObjSet(db: Db, reader: *Obj, prop: PropsEnum, allocator: std.mem.Allocator) !?[]const ObjId {
            return db.readSubObjSet(reader, Self.propIdx(prop), allocator);
        }

        pub fn removeFromSubObjSet(db: Db, writer: *Obj, prop: PropsEnum, sub_writer: *Obj) !void {
            try db.removeFromSubObjSet(writer, Self.propIdx(prop), sub_writer);
        }

        pub fn createBlob(db: Db, writer: *Obj, prop: PropsEnum, size: usize) anyerror!?[]u8 {
            return try db.createBlob(writer, Self.propIdx(prop), size);
        }
        pub fn readBlob(db: Db, reader: *Obj, prop: PropsEnum) []u8 {
            return db.readBlob(reader, Self.propIdx(prop));
        }

        pub fn isInSet(db: Db, reader: *Obj, prop: PropsEnum, item_ibj: ObjId) bool {
...
pub const Db = struct {
    ...
    pub fn createObject(self: Self, type_idx: TypeIdx) anyerror!ObjId {
    ...
    pub fn instantiateSubObj(self: Self, writer: *Obj, prop_idx: u32) !void {
        try self.vtable.instantiateSubObjFn(self.ptr, writer, prop_idx);
    }
    ...
    pub fn readStr(self: Self, reader: *Obj, prop_idx: u32) ?[:0]const u8 {
        const value_ptr = self.vtable.readGenericFn(self.ptr, reader, prop_idx, PropType.STR);
    ...
    pub fn readRefSetAdded(self: Self, reader: *Obj, prop_idx: u32, allocator: std.mem.Allocator) ?[]const ObjId {
        return self.vtable.readRefSetAddedFn(self.ptr, reader, prop_idx, allocator);
    }
    ...
    pub fn readRefSetRemoved(self: Self, reader: *Obj, prop_idx: u32, allocator: std.mem.Allocator) ?[]const ObjId {
        return self.vtable.readRefSetRemovedFn(self.ptr, reader, prop_idx, allocator);
    }
    ...
    pub fn readSubObjSetAdded(self: Self, reader: *Obj, prop_idx: u32, allocator: std.mem.Allocator) !?[]const ObjId {
        return self.vtable.readSubObjSetAddedFn(self.ptr, reader, prop_idx, allocator);
    }
    ...
    pub fn readSubObjSetRemoved(self: Self, reader: *Obj, prop_idx: u32, allocator: std.mem.Allocator) ?[]const ObjId {
        return self.vtable.readSubObjSetRemovedFn(self.ptr, reader, prop_idx, allocator);
    }
    ...
    //TODO: temporary
    pub fn stressIt(self: Self, type_idx: TypeIdx, type_hash2: TypeIdx, ref_obj1: ObjId) anyerror!void {
    ...
    pub fn getDefaultObject(self: Self, type_idx: TypeIdx) ?ObjId {
        return self.vtable.getDefaultObjectFn(self.ptr, type_idx);
    }

    pub fn getFirstObject(self: Self, type_idx: TypeIdx) ?ObjId {
        return self.vtable.getFirstObjectFn(self.ptr, type_idx);
    }

    pub fn getAllObjectByType(self: Self, allocator: std.mem.Allocator, type_idx: TypeIdx) ?[]ObjId {
        return self.vtable.getAllObjectByTypeFn(self.ptr, allocator, type_idx);
    }

    pub fn hasTypeSet(self: Self, type_idx: TypeIdx) bool {
        return self.vtable.hasTypeSetFn(self.ptr, type_idx);
    }

    pub fn hasTypeSubobject(self: Self, type_idx: TypeIdx) bool {
        return self.vtable.hasTypeSubobjectFn(self.ptr, type_idx);
    }

    pub fn getTypeIdx(self: Self, type_hash: strid.StrId32) ?TypeIdx {
    ...
    pub const VTable = struct {
        ...
        instantiateSubObjFn: *const fn (db_: *anyopaque, writer: *Obj, prop_idx: u32) anyerror!void,
        ...
        readRefSetAddedFn: *const fn (db: *anyopaque, reader: *Obj, prop_idx: u32, allocator: std.mem.Allocator) ?[]const ObjId,
        readRefSetRemovedFn: *const fn (db: *anyopaque, reader: *Obj, prop_idx: u32, allocator: std.mem.Allocator) ?[]const ObjId,
        ...
        readSubObjSetAddedFn: *const fn (db: *anyopaque, reader: *Obj, prop_idx: u32, allocator: std.mem.Allocator) ?[]const ObjId,
        readSubObjSetRemovedFn: *const fn (db: *anyopaque, reader: *Obj, prop_idx: u32, allocator: std.mem.Allocator) ?[]const ObjId,
...
pub const CdbAPI = struct {
    ...
    /// Create new database.
    pub fn createDb(self: *Self, name: [:0]const u8) !Db {
        return try self.createDbFn(name);
    }

    /// Destroy and free database.
    pub fn destroyDb(self: *Self, db: Db) void {

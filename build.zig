...
pub fn build(b: *std.Build) !void {
    ...
    // Gen IDE config
    //
    const gen_ide_step = b.step("gen-ide", "init/update vscode configs");
    const gen_ide = b.addRunArtifact(generate_ide_tool);
    gen_ide.addArg(@tagName(options.ide));
    ...
    kernel_module.addAnonymousImport("fa-regular-400", .{
        .root_source_file = b.path("externals/shared/fonts/fa-regular-400.ttf"),
    });
    ...
    tests.root_module.addImport("zjobs", zjobs.module("root"));
    ...
    inline for (.{ tests, lib }) |e| {
        ...
        e.root_module.addAnonymousImport("fa-regular-400", .{
            .root_source_file = b.path("externals/shared/fonts/fa-regular-400.ttf"),
        });

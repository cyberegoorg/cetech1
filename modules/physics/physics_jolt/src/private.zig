...
const physics_system_jolt_c = ecs.ComponentI.implement(
    ...
    struct {
        ...
        //     const components = it.field(PhysicsSystemJolt, 0).?;
        //     _ = components;
        // }
        ...
        pub fn debugdraw(gpu_backend: gpu.GpuBackend, dd: gpu.DDEncoder, world: ecs.World, entites: []const ecs.EntityId, data: []const u8, size: [2]f32) !void {
            ...
            const system = world.getSingletonComponent(PhysicsSystemJolt) orelse return;
...
const physics_shape_jolt_c = ecs.ComponentI.implement(
    ...
    struct {
        ...
        //     if (w.getSingletonComponent(PhysicsSystemJolt)) |system| {
        //         const components = it.field(PhysicsShapeJolt, 0).?;
        //         _ = system;
        //         _ = components;

        //         // var bi = system.system.?.getBodyInterfaceMut();
        //         // for (components) |component| {
        //         //     bi.removeAndDestroyBody(component.body);
        //         // }
        //     }
        // }
    },
...
const physics_body_jolt_c = ecs.ComponentI.implement(
    ...
    .{},
    struct {
        // pub fn onDestroy(components: []PhysicsBodyJolt) !void {
        //     for (components) |c| {
        //         if (c.body == .invalid) continue;
        //     }
        // }

        // pub fn onMove(dsts: []PhysicsBodyJolt, srcs: []PhysicsBodyJolt) !void {
        //     for (dsts, srcs) |*dst, *src| {
        //         dst.* = src.*;
        ...
        //     if (w.getSingletonComponent(PhysicsSystemJolt)) |system| {
        //         const components = it.field(PhysicsBodyJolt, 0).?;

        //         // If PhysicsBodyJolt change this make user unhappy.
        //         var body_ids: []zphy.BodyId = undefined;
        //         body_ids.ptr = @ptrCast(components.ptr);
        //         body_ids.len = components.len;

        //         var bi = system.system.?.getBodyInterfaceMut();
        //         bi.removeBodies(body_ids);
        //         bi.destroyBodies(body_ids);
        //     }
        // }
    },
);
...
const init_physics_body_system_i = ecs.SystemI.implement(
    .{
        ...
        .query = &.{
            ...
            .{ .id = ecs.id(transform.Transform), .inout = .In },
    ...
    struct {
        pub fn iterate(world: ecs.World, it: *ecs.Iter, dt: f32) !void {
            ...
            while (it.next()) {
                ...
                const transforms = it.field(transform.Transform, 2).?;
                ...
                for (0..it.count()) |idx| {
                    ...
                    // const rot = zm.util.getRotationQuat(t.mtx);
                    ...
                    const body = try body_interface.?.createBody(.{
                        ...
                        .position = .{ t.position.x, t.position.y, t.position.z, 0 },
                        .rotation = zm.vecToArr4(t.rotation.q),
    ...
    switch (physics_shape.type) {
        .box => {
            const shape_settings = try zphy.BoxShapeSettings.create(
                .{ physics_shape.size[0], physics_shape.size[1], physics_shape.size[2] },
            );
        ...
        .sphere => {
            const shape_settings = try zphy.SphereShapeSettings.create(physics_shape.size[0]);
            defer shape_settings.asShapeSettings().release();
...
const init_physics_shape_system_i = ecs.SystemI.implement(
    ...
    struct {
        pub fn update(world: ecs.World, it: *ecs.Iter, dt: f32) !void {
            ...
            const ents = it.entities();
            //var physics_system = &it.field(PhysicsSystemJolt, 1).?[0];
...
const simulate_system_i = ecs.SystemI.implement(
    .{
        ...
        .query = &.{
            .{
                .id = ecs.id(physics.PhysicsSystem),
                .inout = .In,
            },
            .{
                .id = ecs.id(PhysicsSystemJolt),
                .inout = .In,
            },
        },
    },
...
const sync_body_system_i = ecs.SystemI.implement(
    .{
        .name = "physics_jolt.sync_body_system",
        .multi_threaded = true,
        .simulation = true,
        .phase = ecs.OnValidate,
        .query = &.{
            .{ .id = ecs.id(PhysicsSystemJolt), .inout = .In },
            .{ .id = ecs.id(PhysicsBodyJolt), .inout = .In },
            .{ .id = ecs.id(transform.Transform), .inout = .Out },
        },
    },
    struct {
        pub fn update(world: ecs.World, it: *ecs.Iter, dt: f32) !void {
            ...
            const ents = it.entities();
            const physics_system = &it.field(PhysicsSystemJolt, 0).?[0];
            const bodies = it.field(PhysicsBodyJolt, 1).?;
            const transforms = it.field(transform.Transform, 2).?;

            _ = ents;

            const all_bodies = physics_system.system.?.getBodiesUnsafe();

            for (0..it.count()) |idx| {
                const body = zphy.tryGetBody(all_bodies, bodies[idx].body).?;
                if (body.motion_type == .static) continue;

                const pos = body.getPosition();
                const rot = body.getRotation();

                transforms[idx].position = .{ .x = pos[0], .y = pos[1], .z = pos[2] };
                transforms[idx].rotation = .{ .q = rot };
            }
        }
    },
);
    ...
    struct {
        pub fn update(world: ecs.World, it: *ecs.Iter, dt: f32) !void {
            ...
            for (0..it.count()) |idx| {
                ...
                world.removeComponent(PhysicsShapeJolt, ents[idx]);
            }
        }
    },
);
...
const transform_changed_observer_i = ecs.ObserverI.implement(
    ...
    struct {
        pub fn update(world: ecs.World, it: *ecs.Iter, dt: f32) !void {
            ...
            const transforms = it.field(transform.Transform, 0).?;
            ...
            _ = world;
            for (0..it.count()) |idx| {
                const body_id = bodies[idx].body;
                const pos = transforms[idx].position;
                const rot = transforms[idx].rotation.q;

                bi.setPosition(body_id, .{ pos.x, pos.y, pos.z }, .activate);
                bi.setRotation(body_id, .{ rot[0], rot[1], rot[2], rot[3] }, .activate);
            }
        }
    },
);
...
const DebugRenderer = if (!debug_renderer_enabled) void else extern struct {
    ...
    pub fn drawLine(
        ...
        if (self.dd) |dd| {
            ...
            dd.setTransform(&zm.matToArr(zm.identity()));
            dd.moveTo(from.*);
            dd.lineTo(to.*);
        }
    }
    ...
    pub fn drawGeometry(
        ...
        if (self.dd) |dd| {
            ...
            dd.setColor(color.uint);

            dd.setState(false, false, false);
            dd.setWireframe(draw_mode == .draw_mode_wireframe);
            dd.setTransform(model_matrix);
...
pub fn load_module_zig(apidb: *const cetech1.apidb.ApiDbAPI, allocator: Allocator, log_api: *const cetech1.log.LogAPI, load: bool, reload: bool) anyerror!bool {
    ...
    try apidb.implOrRemove(module_name, ecs.SystemI, &sync_body_system_i, load);

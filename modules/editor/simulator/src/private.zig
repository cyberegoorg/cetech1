...
var foo_tab = editor.TabTypeI.implement(editor.TabTypeIArgs{
    ...
    pub fn menu(inst: *editor.TabO) !void {
        ...
        if (_coreui.beginMenu(allocator, cetech1.coreui.Icons.Explosion, true, null)) {
            ...
            _ = _coreui.dragU32("count", .{ .v = &tab_o.explode_num });

            if (_coreui.menuItem(allocator, cetech1.coreui.Icons.Explosion, .{}, null)) {
                if (tab_o.root_entity) |root_ent| {
                    tab_o.world.destroyEntities(&.{root_ent});
                }
                ...
                _ = tab_o.world.setId(transform.Position, light_ent, &transform.Position{ .y = 20 });

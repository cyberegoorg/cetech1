...
var foo_tab = editor.TabTypeI.implement(editor.TabTypeIArgs{
    ...
    pub fn create(tab_id: u32) !?*editor.TabI {
        ...
        _ = w.setComponent(transform.TransformComponent, camera_ent, &transform.TransformComponent{ .local = .{ .position = .{ .y = 2, .z = -12 } } });
    ...
    pub fn menu(inst: *editor.TabO) !void {
        ...
        if (_coreui.beginMenu(allocator, cetech1.coreui.Icons.Explosion, true, null)) {
            ...
            if (_coreui.button(cetech1.coreui.Icons.Explosion, .{})) {
                ...
                _ = tab_o.world.setComponent(transform.TransformComponent, light_ent, &transform.TransformComponent{ .local = .{ .position = .{ .y = 20 } } });

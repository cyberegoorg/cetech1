...
var asset_preview_tab = editor.TabTypeI.implement(editor.TabTypeIArgs{
    ...
    pub fn create(tab_id: u32) !?*editor.TabI {
        ...
        _ = w.setComponent(transform.TransformComponent, camera_ent, &transform.TransformComponent{ .local = .{ .position = .{ .y = 2, .z = -5 } } });
        ...
        if (!tab_o.selection.top_level_obj.eql(selected.top_level_obj)) {
            ...
            if (tab_o.root_entity) |ent| {
                ...
                _ = tab_o.world.setComponent(
                    ...
                    tab_o.camera_ent,
                    &transform.TransformComponent{ .local = .{ .position = .{ .y = 2, .z = -5 } } },
                );
            }

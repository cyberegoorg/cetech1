...
const test_geometry_type_aspec = editor_inspector.UiPropertyAspect.implement(struct {
    pub fn ui(
        ...
        _ = allocator; // autofix
        _ = args; // autofix
        const r = renderer.TestGeometryNodeSettings.read(_cdb, obj).?;
        const type_str = renderer.TestGeometryNodeSettings.readStr(_cdb, r, .type) orelse "cube";
        var type_enum = std.meta.stringToEnum(renderer.TestGeometryNodeType, type_str).?;

        try _inspector.uiPropInputBegin(obj, prop_idx, true);
        defer _inspector.uiPropInputEnd();

        if (_coreui.comboFromEnum("", &type_enum)) {
            const w = renderer.TestGeometryNodeSettings.write(_cdb, obj).?;

            try renderer.TestGeometryNodeSettings.setStr(_cdb, w, .type, @tagName(type_enum));
            try renderer.TestGeometryNodeSettings.commit(_cdb, w);
        }
    }
});
...
const post_create_types_i = cdb.PostCreateTypesI.implement(struct {
    pub fn postCreateTypes(db: cdb.DbId) !void {
        try renderer.TestGeometryNodeSettings.addPropertyAspect(
...
pub fn load_module_zig(apidb: *const cetech1.apidb.ApiDbAPI, allocator: Allocator, log_api: *const cetech1.log.LogAPI, load: bool, reload: bool) anyerror!bool {
    ...
    _g.test_geometry_type_aspec = try apidb.globalVarValue(editor_inspector.UiPropertyAspect, module_name, "ct_test_geometry_type_aspec", test_geometry_type_aspec);

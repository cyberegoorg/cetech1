...
const ASSET_BROWSER_ICON = coreui.Icons.AssetBrowser;
...
const api = public.AssetBrowserAPI{};
...
var asset_browser_tab = editor_tabs.TabTypeI.implement(editor_tabs.TabTypeIArgs{
    ...
    pub fn create(tab_id: u32) !?*editor_tabs.TabI {
        ...
        tab_inst.* = AssetBrowserTab{
            ...
            .tags = try assetdb.TagsCdb.createObject(assetdb.getDb()),
            .selection_obj = coreui.Selection.init(_allocator),
            .horizontal_state = .{
                .selected_folder = assetdb.getRootFolder(),
            },
        };
    ...
    pub fn menu(inst: *editor_tabs.TabO) !void {
        ...
        if (coreui.beginMenu(allocator, coreui.Icons.ContextMenu ++ "###ObjContextMenu", selected_count != 0, null)) {
            defer coreui.endMenu();
            editor.showObjContextMenu(
                allocator,
                tab_o,
                MAIN_CONTEXTS,
                first_selected_obj,
        ...
        if (coreui.beginMenu(allocator, ASSET_BROWSER_ICON ++ "###AssetBrowserMenu", true, null)) {
            defer coreui.endMenu();

            coreui.separatorText("Browser type");

            inline for (@typeInfo(BrowserType).@"enum".fields) |f| {
                const value: BrowserType = @enumFromInt(f.value);
                var checked = tab_o.type == value;
                if (coreui.checkbox(f.name ++ "###" ++ f.name, .{ .v = &checked })) {
                    tab_o.type = value;
                }
            }
        }

        if (coreui.beginMenu(allocator, coreui.Icons.AddAsset ++ "###AddAsset", selected_count != 0, null)) {
            defer coreui.endMenu();
    ...
    pub fn ui(inst: *editor_tabs.TabO, kernel_tick: u64, dt: f32) !void {
        ...
        const root_folder = assetdb.getRootFolder();
        if (root_folder.isEmpty()) {
            coreui.text("No root folder");
            return;
        }
    ...
    pub fn assetRootOpened(inst: *editor_tabs.TabO) !void {
        ...
        tab_o.horizontal_state = AssetBrowserState{
            .selected_folder = assetdb.getRootFolder(),
        };
    }

    pub fn selectObjFromMenu(allocator: std.mem.Allocator, tab: *editor_tabs.TabO, ignored_obj: cdb.ObjId, allowed_type: cdb.TypeIdx, enabled: bool) !cdb.ObjId {
        ...
        if (cdb.readObj(selected_obj.obj)) |r| {
            if (selected_obj.obj.type_idx.eql(AssetTypeIdx)) {
                real_obj = assetdb.AssetCdb.readSubObj(r, .Object).?;

                var buff: [1024]u8 = undefined;
                const path = assetdb.getFilePathForAsset(&buff, selected_obj.obj) catch undefined;
        ...
        if (coreui.beginMenu(allocator, coreui.Icons.Select ++ "  " ++ "From" ++ "###SelectFrom", enabled, null)) {
            defer coreui.endMenu();

            if (coreui.menuItem(allocator, label, .{ .enabled = valid }, null)) {
                return real_obj;
            }
        }
...
fn filterType(allocator: std.mem.Allocator, tab: *editor_tabs.TabO, db: cdb.DbId, allways_folder: bool) !void {
    ...
    if (coreui.beginPopup("asset_browser_filter_popup", .{})) {
        defer coreui.endPopup();
        const impls = try apidb.getImpl(allocator, editor_assetdb.CreateAssetI);
        defer allocator.free(impls);

        const folder_type_idx = assetdb.FolderCdb.typeIdx(db);

        if (coreui.menuItem(allocator, "All", .{}, null)) {
            tab_o.type_filter.clearRetainingCapacity();
        }
        ...
        for (impls) |iface| {
            ...
            const type_idx = cdb.getTypeIdx(db, iface.cdb_type).?;
            const type_name = cdb.getTypeName(db, type_idx).?;
            const label = try std.fmt.bufPrintZ(&buff, "{s}###{s}", .{ menu_name, type_name });

            var selected = tab_o.type_filter.contains(type_idx);

            if (coreui.menuItemPtr(allocator, label, .{ .selected = &selected }, null)) {
                if (selected) {
    ...
    if (coreui.button(coreui.Icons.Filter ++ "###FilterAssetByType", .{})) {
        coreui.openPopup("asset_browser_filter_popup", .{});
    }
}

const AssetBrowserState = struct {
    ...
    pub fn updateLayoutSizes(self: *AssetBrowserState, count: u32, avail_width: f32) void {
        // Layout: when not stretching: allow extending into right-most spacing.
        self.layout_item_spacing = @floatFromInt(self.icon_spacing);

        self.card_bottom_height = @as(f32, @floatFromInt(self.card_bottom_row_count)) * coreui.getFontSize();
...
fn uiAssetCard(
    ...
    const db = cdb.getDbFromObjid(item_obj);
    const is_folder = assetdb.isAssetFolder(item_obj);

    const asset_r = cdb.readObj(item_obj).?;
    const asset_name = assetdb.AssetCdb.readStr(asset_r, .Name) orelse "empty name";
    const obj = assetdb.getObjForAsset(item_obj).?;
    const type_name = cdb.getTypeName(db, obj.type_idx).?;

    const asset_color = editor.getAssetColor(item_obj);
    const defualt_color = coreui.getStyle().getColor(.text);

    const selected_item = coreui.SelectedObj{ .top_level_obj = item_obj, .obj = item_obj };
    const item_is_selected = selections.isSelected(.{ .top_level_obj = item_obj, .obj = item_obj });

    coreui.pushStyleVar2f(.{
        .idx = .item_spacing,
    ...
    });
    {
        ...
        const asset_label = try editor.formatObjLabel(allocator, item_obj, null, .{ .with_id = true });
        defer allocator.free(asset_label);

        if (coreui.selectable(asset_label, .{
            .selected = item_is_selected,
            .flags = .{},
            .w = state.layout_item_size.x,
            .h = state.layout_item_size.y,
        })) {
            try coreui.handleSelection(allocator, selections, selected_item, false);
            editor_tabs.propagateSelection(tab, &.{selected_item});
        }
    }
    const item_is_visible = coreui.isItemVisible(); //coreui.isRectVisible(state.layout_item_size);

    if (coreui.beginPopupContextItem()) {
        defer coreui.endPopup();
        try editor.showObjContextMenu(allocator, tab, MAIN_CONTEXTS, selected_item);
    }

    if (coreui.isItemHovered(.{}) and coreui.isMouseDoubleClicked(.left)) {
        try editor_obj_buffer.addToFirst(allocator, db, selected_item);
        if (is_folder) {
            state.selected_folder = item_obj;
        }
    }

    try editor.uiAssetDragDropSource(allocator, item_obj);
    try editor.uiAssetDragDropTarget(allocator, tab, item_obj, null);

    const visual_aspect = cdb.getAspect(editor.UiVisualAspect, db, item_obj.type_idx);

    if (item_is_visible) {
        const card_min: math.Vec2f = .{ .x = pos.x, .y = pos.y };
        const card_max: math.Vec2f = .{ .x = card_min.x + state.layout_item_size.x, .y = card_min.y + state.layout_item_size.y };
        const card_size: math.Vec2f = state.layout_item_size;
        const card_size_half: math.Vec2f = card_size.div(.splat(2));

        const icon_bg_color = math.SRGBA{ .r = 59, .g = 59, .b = 59, .a = 255 };
        const bottom_bg_color = math.SRGBA{ .r = 48, .g = 48, .b = 48, .a = 255 };
        ...
        if (visual_aspect) |aspect| {
            ...
            if (aspect.ui_status_icons) |icons| {
                ...
                coreui.pushFontSize(8);
                defer coreui.popFontSize();
            ...
            if (aspect.ui_icons) |icons| {
                var icon_buf: [16:0]u8 = undefined;
                const icon = try icons(&icon_buf, allocator, item_obj);
                const icon_size = @max(state.icon_size.x, state.icon_size.y) * 0.6;
                coreui.pushFontSize(icon_size);
                defer coreui.popFontSize();

                const font_size = coreui.calcTextSize(icon, .{});
                const half_font = font_size.div(.splat(2));

                draw_list.addTextUnformatted(
                    .{
                        .x = (card_min.x + (state.icon_size.x / 2)) - half_font.x,
                        .y = (card_min.y + (state.icon_size.y / 2)) - half_font.y,
                    },
                    .fromColor4f(asset_color),
                    icon,
                );
        ...
        if (display_label) {
            // Asset name
            const max_char_in_line: usize = @intFromFloat(@floor(state.layout_item_size.x / (coreui.getFontSize() / 2)));
                ...
                if (!is_folder) {
                    draw_list.addTextUnformatted(
                        .{
                            ...
                            .y = (card_min.y + state.icon_size.y) + state.text_boreder_spacing + (coreui.getFontSize() * @as(f32, @floatFromInt(line_idx))),
                        },
                        .fromColor4f(defualt_color),
                        txt,
                    );
                } else {
                    const t_size = coreui.calcTextSize(txt, .{});
                    draw_list.addTextUnformatted(
                        .{
                            .x = card_min.x + card_size_half.x - t_size.x / 2,
                            .y = (card_min.y + state.icon_size.y) + (coreui.getFontSize() * @as(f32, @floatFromInt(line_idx))),
                        },
                        .fromColor4f(defualt_color),
                        txt,
                    );
            ...
            try uiDrawTags(
                ...
                .{
                    .x = card_min.x + state.text_boreder_spacing,
                    .y = (card_min.y + state.icon_size.y) - coreui.getFontSize(),
                },
                draw_list,
            ...
            if (!is_folder) {
                ...
                draw_list.addTextUnformatted(
                    .{
                        .x = card_min.x + state.text_boreder_spacing,
                        .y = (card_max.y - state.text_boreder_spacing - coreui.getFontSize()),
                    },
                    .fromColor4f(defualt_color),
                    type_name,
                );
...
fn uiDrawTags(allocator: std.mem.Allocator, obj: cdb.ObjId, pos: math.Vec2f, draw_list: coreui.DrawList, tag_prop_idx: u32) !void {
    ...
    if (cdb.readRefSet(obj_r, tag_prop_idx, allocator)) |tags| {
        ...
        const icon_size = coreui.calcTextSize(tag_lbl, .{});
        ...
        for (tags, 0..) |tag, idx| {
            ...
            const mouse_pos = coreui.getMousePos();
...
fn uiAssetBrowserCards(
    ...
    var start_pos = coreui.getCursorScreenPos();
    start_pos = .{
        .x = start_pos.x + state.layout_outer_padding,
        .y = start_pos.y + state.layout_outer_padding,
    };
    coreui.setCursorScreenPos(start_pos);

    const column_count = state.layout_column_count;
    const draw_list = coreui.getWindowDrawList();

    var clipper = coreui.createClipper();
    clipper.begin(@intCast(state.layout_line_count), state.layout_item_step.y);
    defer clipper.end();

    while (clipper.step()) {
        for (@intCast(clipper.display_start)..@intCast(clipper.display_end)) |line_idx| {
...
const FilterBarResult = struct {
    new_filter: ?[:0]const u8 = null,
    tag_filter_used: bool = false,
};
...
fn uiFilterBar(
    ...
    if (coreui.beginChild(
        ...
        coreui.sameLine(.{});
        tag_filter_used = try editor_assetdb.tagsInput(allocator, tags_filter, assetdb.TagsCdb.propIdx(.Tags), false, null);

        coreui.sameLine(.{});
        new_filter = coreui.uiFilter(filter_buff, filter);
    }

    return .{ .new_filter = new_filter, .tag_filter_used = tag_filter_used };
}
...
fn uiAssetBrowser(
    ...
    var filter_tab_result = FilterBarResult{};

    // Filter tab
    // if (btype == .Vertical) {
    filter_tab_result = try uiFilterBar(allocator, tab, root_folder, tags_filter, args.filter, filter_buff);
    // }

    if (filter_tab_result.new_filter == null and !filter_tab_result.tag_filter_used) {
        ...
        );
        {
            ...
            if (coreui.beginChild(
                folder_child_label,
                .{
                    .w = if (btype == .Horizontal) 170 else 0,
                    .h = if (btype == .Vertical) 20 else 0,
                    .child_flags = .{
                        .border = true,
                        .resize_x = btype == .Horizontal,
                        .resize_y = btype == .Vertical,
                    },
                    .window_flags = .{ .no_saved_settings = true },
                },
            )) {
                ...
                const new_selected = try editor_tree.cdbTreeView(
                    allocator,
                    tab,
                    contexts,
                ...
                if (new_selected) {
                    ...
                    try coreui.handleSelection(allocator, selections, s[0], false);
                    // editor_tabs.propagateSelection(tab, s);
                }
            }
        }
        ...
        if (coreui.beginChild(
            ...
            // Content tab
            {
                ...
                if (coreui.beginChild(
                    ...
                    // Filter
                    if (filter_tab_result.new_filter != null or filter_tab_result.tag_filter_used) {
                        if (filter_tab_result.new_filter) |f| {
                            result.filter = f;
                        }
                        ...
                        if (filter_tab_result.new_filter != null and isUuid(filter_tab_result.new_filter.?)) {
                            if (uuid.fromStr(filter_tab_result.new_filter.?)) |filter_uuid| {
                                if (assetdb.getObjId(filter_uuid)) |asset| {
                                    try uiAssetBrowserCards(
                                        ...
                                        selections,
                                        state,
                                        coreui.getContentRegionAvail().x,
                                    );
                                }
                            }
                        } else {
                            const assets_filtered = assetdb.filerAsset(allocator, if (args.filter) |f| f else "", tags_filter) catch undefined;
                            ...
                            for (assets_filtered) |asset| {
                                if (!editor_assetdb.filterOnlyTypes(args.only_types, assetdb.getObjForAsset(asset.obj).?)) continue;
                        ...
                        for (set) |ref_obj| {
                            if (ref_obj.type_idx.eql(AssetTypeIdx)) {
                                if (!editor_assetdb.filterOnlyTypes(args.only_types, assetdb.getObjForAsset(ref_obj).?)) continue;
        ...
        _ = coreui.registerTest(
            ...
            struct {
                pub fn run(ctx: *coreui.TestContext) !void {
                    ...
                    ctx.itemInputStrValue("**/###filter", "foo");

                    ctx.itemAction(.DoubleClick, "**/###foo.ct_foo_asset", .{}, null);
                    ctx.itemAction(.DoubleClick, "**/###foo_core.ct_foo_asset", .{}, null);
                    ctx.itemAction(.DoubleClick, "**/###foo_core2.ct_foo_asset", .{}, null);
                    ctx.itemAction(.DoubleClick, "**/###foo_subcore.ct_foo_asset", .{}, null);
                    ctx.itemAction(.DoubleClick, "**/###core_subfolder", .{}, null);
                }
            },
        );
        ...
        _ = coreui.registerTest(
            ...
            struct {
                pub fn run(ctx: *coreui.TestContext) !void {
                    ...
                    ctx.itemInputStrValue("**/###filter", "018b5c74-06f7-740e-be81-d727adec5fb4");
        ...
        _ = coreui.registerTest(
            ...
            struct {
                pub fn run(ctx: *coreui.TestContext) !void {
                    ...
                    //ctx.itemAction(  .Click, "**/###asset_a.ct_foo_asset", .{}, null);
                    ctx.yield(1);
...
pub fn load_module_zig(allocator: Allocator, load: bool, reload: bool) anyerror!bool {
    ...
    try editor_assetdb.loadAPI(module_name);

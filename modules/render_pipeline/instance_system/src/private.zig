...
fn createInstanceSystem(mtxs: []const math.Mat44f) !public.InstanceSystem {
    const instance_system = _shader_system.findSystemByName(.fromStr("instance_system")).?;
    const system_io = _shader_system.getSystemIO(instance_system);

    const instance_system_uniforms = (try _shader_system.createUniformBuffer(system_io)).?;
    const instance_system_resources = (try _shader_system.createResourceBuffer(system_io)).?;
    ...
    try _shader_system.updateUniforms(system_io, instance_system_uniforms, &.{
        .{

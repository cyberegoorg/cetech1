...
const vec3 color = texture2D(get_hdr_sampler(), tex_coord).rgb;

const vec3 hdr_color = params.bloom_enabled ? mix(color, texture2D(get_bloom_sampler(), tex_coord).rgb, 0.04) : color;
...
outputs.color0 = tonemaped_color;

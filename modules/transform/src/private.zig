...
const api = public.TransformApi{
    ...
    .transformChilds = transform_childs,
};

const transform_c = ecs.ComponentI.implement(public.TransformComponent, .{
    .cdb_type_hash = public.TransformComponentCdb.type_hash,
    .category = "Transform",
    .category_order = 0.3,
    .gizmoPriority = 100,
    ...
    pub fn fromCdb(
        ...
        const transform = std.mem.bytesAsValue(public.TransformComponent, data);

        const pos_obj = public.TransformComponentCdb.readSubObj(_cdb, t, .Position).?;
        ...
        const rot_obj = public.TransformComponentCdb.readSubObj(_cdb, t, .Rotation).?;
        ...
        const scale_obj = public.TransformComponentCdb.readSubObj(_cdb, t, .Scale).?;
    ...
    pub fn gizmoGetMatrix(
        ...
        const t = world.getComponent(public.TransformComponent, entity) orelse return;
    ...
    pub fn gizmoSetMatrix(
        ...
        const comp_r = public.TransformComponentCdb.read(_cdb, component_obj).?;
        const pos_obj = public.TransformComponentCdb.readSubObj(_cdb, comp_r, .Position).?;
        const rot_obj = public.TransformComponentCdb.readSubObj(_cdb, comp_r, .Rotation).?;
        const scl_obj = public.TransformComponentCdb.readSubObj(_cdb, comp_r, .Scale).?;
...
const world_tranform_c = cetech1.ecs.ComponentI.implement(public.WorldTransformComponent, .{}, struct {});
...
fn transform_entity_parented(world: ecs.World, entity: ecs.EntityId, parent_transform: ?math.Transform, only_child: bool) void {
    const local_transform = world.getComponent(public.TransformComponent, entity).?;
...
const transform_system_i = ecs.SystemI.implement(
    .{
        ...
        .query = &.{
            ...
            .{ .id = ecs.id(public.TransformComponent), .inout = .In },
        },
    },
    struct {
...
const set_position_node_i = graphvm.NodeI.implement(
    ...
    struct {
        ...
        pub fn execute(self: *const graphvm.NodeI, args: graphvm.ExecuteArgs, in_pins: graphvm.InPins, out_pins: *graphvm.OutPins) !void {
            ...
            var t = world.getMutComponent(public.TransformComponent, ent).?;
...
const transform_set_observer_i = ecs.ObserverI.implement(
    .{
        .name = "transform.transform_add_observer_i",
        .query = &.{
            .{ .id = ecs.id(public.TransformComponent), .inout = .In },
    ...
    pub fn createTypes(db: cdb.DbId) !void {
        ...
        // Transform
        {
            const idx = try _cdb.addType(
                db,
                public.TransformComponentCdb.name,
                &[_]cdb.PropDef{
                    .{
                        .prop_idx = public.TransformComponentCdb.propIdx(.Position),
                        .name = "position",
                        .type = .SUBOBJECT,
                        .type_hash = cdb_types.PositionCdb.type_hash,
                    },
                    .{
                        .prop_idx = public.TransformComponentCdb.propIdx(.Rotation),
                        .name = "rotation",
                        .type = .SUBOBJECT,
                        .type_hash = cdb_types.RotationCdb.type_hash,
                    },
                    .{
                        .prop_idx = public.TransformComponentCdb.propIdx(.Scale),
                        .name = "scale",
                        .type = .SUBOBJECT,
            ...
            try public.TransformComponentCdb.setSubObj(_cdb, default_quat_w, .Position, default_pos_w);
            ...
            try public.TransformComponentCdb.setSubObj(_cdb, default_quat_w, .Rotation, default_rot_w);
            ...
            try public.TransformComponentCdb.setSubObj(_cdb, default_quat_w, .Scale, default_scale_w);

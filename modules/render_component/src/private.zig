        ...
        if (_gpu.getEncoder()) |e| {
            const dd = viewport.getDD();
            {
                ...
                if (culling_result) |result| {
                    ...
                    // TODO: From shader
                    const state = 0 | gpu.StateFlags_WriteRgb | gpu.StateFlags_WriteA | gpu.StateFlags_WriteZ | gpu.StateFlags_DepthTestLess | gpu.StateFlags_CullCcw | gpu.StateFlags_Msaa;
                    for (draw_calls, result.mtx.items) |draw_call, mtx| {
                        if (draw_call) |dc| {
                            if (dc.gpu_geometry != null and dc.gpu_index_buffer != null and dc.gpu_shader != null) {
                                ...
                                e.setState(state, 0);
                                const prg = _shader_system.getGpuProgram(dc.gpu_shader.?).?;

...
pub const CullingResult = struct {
    ...
    pub fn compaction(self: *CullingResult, count: usize, out_entites_idx: []usize, in_entites_idx: ?[]const usize) usize {
        ...
        for (0..count) |cullable_idx| {
            if (self.visibility.items[cullable_idx].mask == 0) continue;
    ...
    pub fn exec(self: *@This()) !void {
        ...
        for (self.volumes, 0..) |sphere, i| {
            ...
            for (self.frustums, 0..) |frustum, frustum_idx| {
                if (sphere.visibility_mask.intersectWith(self.viewers[frustum_idx].visibility_mask).mask == 0) continue;
...
const CullingBoxTask = struct {
    ...
    pub fn exec(self: *@This()) !void {
        ...
        for (self.volumes, 0..) |box, i| {
            ...
            for (self.frustums, 0..) |frustum, frustum_idx| {
                if (box.visibility_mask.intersectWith(self.viewers[frustum_idx].visibility_mask).mask == 0) continue;

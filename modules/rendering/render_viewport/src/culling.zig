...
pub const MatList = cetech1.ArrayList(transform.WorldTransformComponent);
...
pub const MatSliceList = cetech1.ArrayList([]transform.WorldTransformComponent);
...
const CullingSphereTask = struct {
    ...
    pub fn exec(self: *@This()) !void {
        ...
        const frustum = self.viewers[self.viewer_idx].frustum;
        ...
        for (self.volumes, 0..) |sphere, i| {
            ...
            if (sphere.skip_culling or frustum.vsSphereNaive(sphere.center, sphere.radius)) {
...
const CullingBoxTask = struct {
    ...
    pub fn exec(self: *@This()) !void {
        ...
        for (self.volumes, 0..) |box, i| {
            ...
            if (box.skip_culling or frustum.vsOBBNaive(box.t.world, box.min, box.max)) {
                self.result.setVisibility(self.visibility_offset + i, self.viewer_idx, true);
                _ = self.result.visible_cnt.fetchAdd(1, .monotonic);
            }
        }
    ...
    pub fn debugdrawBoundingBoxes(self: *Self, dd: gpu.DDEncoder) !void {
        ...
        for (self.request_map.keys(), self.request_map.values()) |k, rq| {
            ...
            for (rq.box_volumes.items) |box| {
                dd.pushTransform(box.t.world.toMat());

...
ct_spot_light get_spot_light(in uint idx) {
    ...
    p.cos_angle_inner = get_light_system_buffer_buffer_data(offset + 1).w;
    p.cos_angle_outer = get_light_system_buffer_buffer_data(offset + 2).w;
...
vec3 calc_point_light(ct_point_light light, in float dist, in vec3 L, in vec3 V, in vec3 N, in float NoV, in ct_pbr_mat_coef mat_coef) {
...
vec3 pbr_calc_out_radiance(in vec3 V, in vec3 N, in vec3 wp, in ct_pbr_material mat) {
    ...
    for (int i = 0; i < point_light_count; i++) {
        ...
        out_rad += calc_point_light(light, dist, L, V, N, NoV, mat_coef);
    }

    for (int i = 0; i < spot_light_count; i++) {
        const ct_spot_light light = get_spot_light(i);

        const vec3 L = normalize(light.pl.position - wp);
        const float dist = distance(light.pl.position, wp);

        const vec3 point_rad = calc_point_light(light.pl, dist, L, V, N, NoV, mat_coef);
        if (!any(point_rad)) continue;

        const float DL = saturate(dot(-L, light.direction));
        const float epsilon = light.cos_angle_inner - light.cos_angle_outer;
        const float spot_attenuation = saturate((DL - light.cos_angle_outer) / epsilon);

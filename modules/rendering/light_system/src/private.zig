...
fn boundingSphere(origin: [3]f32, forward: [3]f32, size: f32, angle: f32) zm.Vec {
    ...
    if (angle > (std.math.pi / 4.0)) {
        center = zm.mulAdd(zm.loadArr3(forward), zm.splat(zm.Vec, std.math.cos(angle) * size), zm.loadArr3(origin));
        radius = std.math.sin(angle) * size;
    } else {
        center = zm.mulAdd(zm.loadArr3(forward), zm.splat(zm.Vec, size / (2.0 * std.math.cos(angle))), zm.loadArr3(origin));
        radius = size / (2.0 * std.math.cos(angle));
    }
...
const DrawCallCusterDef = struct {
    first_idx: usize,
    calls: []const *light_component.Light,
};

fn clusterByLightsType(allocator: std.mem.Allocator, sorted_instances: []*light_component.Light, max_cluster: usize) ![]DrawCallCusterDef {
...
const light_system_shaderable = render_viewport.ShaderableComponentI.implement(light_component.Light, struct {
            ...
            .sphere => {
                ...
                for (lights.items, 0..) |l, idx| {
                    ...
                    switch (l.type) {
                        ...
                        .spot => {
                            // const forward = zm.util.getAxisZ(mat);
                            // const sphere = boundingSphere(center, zm.vecToArr3(forward), l.radius, std.math.degreesToRadians(l.angle_inner));
                            // sphere_out_volumes[idx] = .{
    ...
    pub fn update(
        ...
        const clusters = try clusterByLightsType(allocator, lights.items, @intFromEnum(light_component.LightType.spot) + 1);
        defer allocator.free(clusters);

        var point_count: u32 = 0;
        var spot_count: u32 = 0;
        for (clusters) |cluster| {
            const light_count: u32 = @truncate(cluster.calls.len);
            switch (cluster.calls[0].type) {
        ...
        // Buffer is vec4
        const light_buffer_size = (point_count * 2) + (spot_count * 3);
        ...
        for (clusters) |cluster| {
            for (cluster.calls, 0..) |l, idx| {
                ...
                switch (l.type) {
                    ...
                    .spot => {
                        const dir = zm.util.getAxisZ(t.mtx);
                        gpu_point_lights.appendSliceAssumeCapacity(&.{
                            .{ pos[0], pos[1], pos[2], l.radius },
                            .{ power[0], power[1], power[2], std.math.cos(std.math.degreesToRadians(l.angle_inner)) },
                            .{ dir[0], dir[1], dir[2], std.math.cos(std.math.degreesToRadians(l.angle_outer)) },
                        });
                    },
                }
            }

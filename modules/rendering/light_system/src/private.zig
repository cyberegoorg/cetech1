...
const G = struct {
    ...
    light_buffer_offset: std.atomic.Value(u32) = .init(0),
};
var _g: *G = undefined;
...
fn createLightSystem() !public.LightSystem {
    ...
    try _shader_system.updateResources(
        ...
        &.{.{ .name = light_system_buffer_strid, .value = .{ .dvb = _g.light_buffer } }},
    );
...
const light_system_shaderable = render_viewport.ShaderableComponentI.implement(light_component.Light, struct {
    ...
    pub fn update(
        ...
        if (gpu_point_lights.items.len > 0) {
            ...
            // }
            const size: u32 = @truncate(@sizeOf(GPUPointLight) * gpu_point_lights.items.len);
            offset = _g.light_buffer_offset.fetchAdd(size, .monotonic);
        ...
        const light_system_id = _shader_system.findSystemByName(.fromStr("light_system")).?;
        if (system_context.getSystem(light_system_id)) |ls| {
            const light_system_io = _shader_system.getSystemIO(light_system_id);

            const lights_count: u32 = @truncate(gpu_point_lights.items.len);

            try _shader_system.updateUniforms(light_system_io, ls.uniforms.?, &.{
                .{
...
var update_task = cetech1.kernel.KernelTaskUpdateI.implment(
    ...
    struct {
        pub fn update(kernel_tick: u64, dt: f32) !void {
            ...
            _g.light_buffer_offset.store(0, .monotonic);
        }
    },
);

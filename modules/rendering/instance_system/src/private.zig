...
const MAX_INSTNACE_MTX = 500_000;
...
const vertex_system_channel0_strid = cetech1.strId32("instance_system_mtx");

fn createInstanceSystem(mtxs: []const transform.WorldTransform) !public.InstanceSystem {
    const vertex_system = _shader_system.findSystemByName(.fromStr("instance_system")).?;
    ...
    try _shader_system.updateResources(
        system_io,
        vertex_system_resources,
        &.{.{ .name = vertex_system_channel0_strid, .value = .{ .dvb = _g.instance_mtx_buffer } }},
    );

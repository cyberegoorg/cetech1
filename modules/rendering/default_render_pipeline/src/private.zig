...
const simple_pass = render_graph.PassApi.implement(struct {
    pub fn setup(pass: *render_graph.Pass, builder: render_graph.GraphBuilder) !void {
        try builder.createTexture2D(
            pass,
            "hdr",
...
const bloom_0_pass = render_graph.PassApi.implement(struct {
    pub fn setup(pass: *render_graph.Pass, builder: render_graph.GraphBuilder) !void {
        ...
        try builder.enablePass(pass, "bloom_0_pass");
    }
...
const bloom_downsample_pass_api = render_graph.PassApi.implement(struct {
    pub fn setup(pass: *render_graph.Pass, builder: render_graph.GraphBuilder) !void {
        ...
        try builder.enablePass(pass, pass.name);
    }
...
const tonemap_pass = render_graph.PassApi.implement(struct {
    pub fn setup(pass: *render_graph.Pass, builder: render_graph.GraphBuilder) !void {
        try builder.readTexture(pass, "hdr");
        try builder.readTexture(pass, "bloom_0");
        try builder.writeTexture(pass, render_viewport.ColorResource);
    ...
    pub fn execute(pass: *const render_graph.Pass, builder: render_graph.GraphBuilder, gpu_api: *const gpu.GpuApi, vp_size: [2]f32, viewid: gpu.ViewId) !void {
        ...
        if (gpu_api.getEncoder()) |e| {
            ...
            const bloom_texture = builder.getTexture("bloom_0").?;
...
const rg_i = render_pipeline.RenderPipelineI.implement(struct {
    ...
    pub fn fillModule(pipeline: *anyopaque, module: render_graph.Module) !void {
        ...
        // Geometry passes
        {
            ...
            try render_module.addPass(.{ .name = "simple_pass", .api = &simple_pass });
        ...
        {
            const render_module = try _render_graph.createModule();
            try render_module.addPass(.{ .name = "bloom_0_pass", .api = &bloom_0_pass });
            ...
            // Downsample
            //
            inline for (0..chain_count) |i| {
                try render_module.addPassWithData(
            ...
            inline for (0..chain_count) |i| {
                try render_module.addPassWithData(
                    ...
                    BloomUpsampleParams{
                        .in_texture_name = std.fmt.comptimePrint("bloom_{d}", .{chain_count - i}),
                        .out_texture_name = std.fmt.comptimePrint("bloom_{d}", .{chain_count - i - 1}),
                        .shader = upsample_shader,
                        .bloom_intensity = inst.bloom_intensity,
                    },
        ...
        // Tonemap hdr output
        try module.addPassWithData(TonemapParams, "tonemap", inst.tonemap_params, &tonemap_pass);

        // DD draw after tonemap
        try module.addPass(.{ .name = "dd_pass", .api = &dd_pass });
    }
    ...
    pub fn uiDebugMenuItems(pipeline: *anyopaque, allocator: std.mem.Allocator) void {
        ...
        if (_coreui.beginMenu(allocator, coreui.Icons.RenderPipeline ++ "  " ++ "Bloom", true, null)) {
            ...
            _ = _coreui.dragF32("intensity", .{ .v = &inst.bloom_intensity, .min = 0, .max = std.math.floatMax(f32) });
        }

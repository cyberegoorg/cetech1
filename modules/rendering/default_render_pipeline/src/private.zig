...
const DefaultPipelineInst = struct {
    ...
    depth_system: shader_system.System = undefined,
};
...
const rg_i = render_pipeline.RenderPipelineI.implement(struct {
    pub fn create(allocator: std.mem.Allocator) !*anyopaque {
        ...
        const depth_system = _shader.findSystemByName(cetech1.strId32("depth_system")).?;
    ...
    pub fn begin(pipeline: *anyopaque, context: *shader_system.SystemContext, now_s: f32) !void {
        ...
        try context.addSystem(inst.depth_system, null, null);
...
var kernel_task = cetech1.kernel.KernelTaskI.implement(
    ...
    struct {
        pub fn init() !void {
            ...
            // Depth system
            try _shader.addSystemDefiniton("depth_system", .{
                .raster_state = .{
                    .cullmode = .back,
                },
                .depth_stencil_state = .{
            ...
            try _shader.addShaderDefiniton("node_foo_output", .{
                ...
                .vertex_block = .{
                    ...
                    \\  output.color0 = load_vertex_color(vertex_ctx, input.vertex_id, 0);
                    ,
                },
                .fragment_block = .{
                    .exports = &.{
                    ...
                    .code =
                    \\  #if !CT_SYSTEM_ACTIVATED(depth_system)
                    \\    ct_graph graph;
                    \\    ct_graph_init(graph);
                    \\  
                ...
                .compile = .{
                    ...
                    .configurations = &.{
                        .{
                            .name = "default",
                            .variations = &.{
                                .{ .systems = &.{ "time_system", "viewer_system", "vertex_system" } },
                            },
                        },
                        .{
                            .name = "depth",
                            .variations = &.{
                                .{
                                    .systems = &.{ "time_system", "viewer_system", "vertex_system", "depth_system" },
                                    .depth_stencil_state = .{
                                        .depth_write_enable = true,
                                        .depth_test_enable = true,
                                        .depth_comapre_op = .less,

...
const G = struct {
    ...
    cube_vb: render_viewport.VertexBuffer = .{},
    cube_geometry: render_viewport.GPUGeometry = undefined,

    bunny_pos_vb: gpu.VertexBufferHandle = .{},
    bunny_col_vb: gpu.VertexBufferHandle = .{},

    bunny_vb: render_viewport.VertexBuffer = .{},
    bunny_geometry: render_viewport.GPUGeometry = undefined,
...
var kernel_task = cetech1.kernel.KernelTaskI.implement(
    ...
    struct {
        pub fn init() !void {
            ...
            _g.cube_vb.buffers[0] = _g.cube_pos_vb;
            _g.cube_vb.buffers[1] = _g.cube_col_vb;
            _g.cube_vb.active_channels.set(render_viewport.VertexChannelsNames.Position);
            _g.cube_vb.active_channels.set(render_viewport.VertexChannelsNames.Color);
            _g.cube_vb.channels[render_viewport.VertexChannelsNames.Position].stride = 4;
            _g.cube_vb.channels[render_viewport.VertexChannelsNames.Position].stride = 4;
            _g.cube_vb.channels[render_viewport.VertexChannelsNames.Color].buffer_idx = 1;
            _g.cube_vb.channels[render_viewport.VertexChannelsNames.Color].stride = 4;

            _g.cube_geometry = .{ .vertex_system = try _viewport.createVertexSystemFromVertexBuffer(_g.cube_vb) };
            ...
            _g.bunny_vb.buffers[0] = _g.bunny_pos_vb;
            _g.bunny_vb.buffers[1] = _g.bunny_col_vb;
            _g.bunny_vb.active_channels.set(render_viewport.VertexChannelsNames.Position);
            _g.bunny_vb.active_channels.set(render_viewport.VertexChannelsNames.Color);
            _g.bunny_vb.channels[render_viewport.VertexChannelsNames.Position].stride = 4;
            _g.bunny_vb.channels[render_viewport.VertexChannelsNames.Position].stride = 4;
            _g.bunny_vb.channels[render_viewport.VertexChannelsNames.Color].buffer_idx = 1;
            _g.bunny_vb.channels[render_viewport.VertexChannelsNames.Color].stride = 4;

            _g.bunny_geometry = .{ .vertex_system = try _viewport.createVertexSystemFromVertexBuffer(_g.bunny_vb) };
        ...
        pub fn shutdown() !void {
            ...
            _shader.destroySystemInstance(&_g.cube_geometry.vertex_system);
            _shader.destroySystemInstance(&_g.bunny_geometry.vertex_system);
...
const gpu_geometry_value_type_i = graphvm.GraphValueTypeI.implement(
    ...
    struct {
        pub fn valueFromCdb(allocator: std.mem.Allocator, obj: cdb.ObjId, value: []u8) !void {
            _ = allocator; // autofix
            _ = obj; // autofix
            const v: render_viewport.GPUGeometry = .{ .vertex_system = .{ .system_idx = 0 } };
        ...
        pub fn valueToString(allocator: std.mem.Allocator, value: []const u8) ![:0]u8 {
            return std.fmt.allocPrintZ(allocator, "{any}", .{std.mem.bytesToValue(render_viewport.GPUGeometry, value)});
        }
    },
);
...
const draw_call_node_i = graphvm.NodeI.implement(
    ...
    struct {
        ...
        pub fn create(self: *const graphvm.NodeI, allocator: std.mem.Allocator, state: *anyopaque, node_obj: cdb.ObjId, reload: bool, transpile_state: ?[]u8) !void {
            ...
            _ = allocator; // autofix
            _ = node_obj; // autofix
            const real_state: *render_viewport.DrawCall = @alignCast(@ptrCast(state));
            real_state.* = .{ .shader = .{} };
        }
        ...
        pub fn execute(self: *const graphvm.NodeI, args: graphvm.ExecuteArgs, in_pins: graphvm.InPins, out_pins: graphvm.OutPins) !void {
            _ = self; // autofix
            _ = out_pins;
            var state = args.getState(render_viewport.DrawCall).?;
            _, const shader = in_pins.read(shader_system.ShaderInstance, 0) orelse .{ 0, shader_system.ShaderInstance{} };
            _, const geometry = in_pins.read(render_viewport.GPUGeometry, 1) orelse .{ 0, render_viewport.GPUGeometry{ .vertex_system = .{ .system_idx = 0 } } };
...
const simple_mesh_node_i = graphvm.NodeI.implement(
    ...
    struct {
        ...
        pub fn execute(self: *const graphvm.NodeI, args: graphvm.ExecuteArgs, in_pins: graphvm.InPins, out_pins: graphvm.OutPins) !void {
            ...
            switch (type_enum) {
                .cube => {
                    try out_pins.writeTyped(render_viewport.GPUGeometry, 0, try gpu_geometry_value_type_i.calcValidityHash(&std.mem.toBytes(_g.cube_geometry)), _g.cube_geometry);
                ...
                .bunny => {
                    try out_pins.writeTyped(render_viewport.GPUGeometry, 0, try gpu_geometry_value_type_i.calcValidityHash(&std.mem.toBytes(_g.bunny_geometry)), _g.bunny_geometry);
...
const PosVertex = struct {
    ...
    fn layoutInit() gpu.VertexLayout {
        ...
        _ = _gpu.layoutAdd(&L.posColorLayout, gpu.Attrib.Position, 4, gpu.AttribType.Float, false, false);

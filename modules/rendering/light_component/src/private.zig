...
const light_c = ecs.ComponentI.implement(
    ...
    struct {
        ...
        pub fn fromCdb(
            ...
            light.* = public.Light{
                .type = if (std.mem.eql(u8, type_str, "point")) .point else if (std.mem.eql(u8, type_str, "spot")) .spot else .point,
                .color = color,
                .radius = radius,
                .power = power,
        ...
        pub fn debugdraw(dd: gpu.DDEncoder, world: ecs.World, entites: []const ecs.EntityId, data: []const u8, size: [2]f32) !void {
            ...
            for (entites, lights) |ent, light| {
                ...
                const dir = zm.util.getAxisZ(wt.mtx);
...
const enum_str = "point\x00spot\x00";
...
var light_type_aspec = editor_inspector.UiPropertyAspect.implement(struct {
    pub fn ui(
        ...
        const type_str = public.LightCdb.readStr(_cdb, r, .Type) orelse "point";
        const type_enum: public.LightType = if (std.mem.eql(u8, type_str, "point")) .point else if (std.mem.eql(u8, type_str, "spot")) .spot else .point;

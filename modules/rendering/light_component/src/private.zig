...
const light_c = ecs.ComponentI.implement(
    ...
    struct {
        ...
        pub fn fromCdb(
            ...
            light.* = public.Light{
                .type = if (std.mem.eql(u8, type_str, "point")) .point else .point,
                .color = color,
            };
        ...
        pub fn debugdraw(dd: gpu.DDEncoder, world: ecs.World, entites: []const ecs.EntityId, data: []const u8, size: [2]f32) !void {
            ...
            lights.ptr = @alignCast(@ptrCast(data.ptr));
            ...
            for (entites, lights) |ent, light| {
                ...
                switch (light.type) {
                    .point => {
                        dd.drawCircleAxis(.X, pos_array, 1.0, 0);
                        dd.drawCircleAxis(.Y, pos_array, 1.0, 0);
                        dd.drawCircleAxis(.Z, pos_array, 1.0, 0);
                    },
                }
            }
        }
...
var light_type_aspec = editor_inspector.UiPropertyAspect.implement(struct {
    pub fn ui(
        ...
        const type_str = public.LightCdb.readStr(_cdb, r, .Type) orelse "point";
        const type_enum: public.LightType = if (std.mem.eql(u8, type_str, "point")) .point else .point;
        ...
        if (_coreui.combo("", .{
            ...
            .items_separated_by_zeros = "point\x00",
        })) {
    ...
    pub fn createTypes(db: cdb.DbId) !void {
        // Light
        {
            const scale_idx = try _cdb.addType(
                ...
                &[_]cdb.PropDef{
                    .{ .prop_idx = public.LightCdb.propIdx(.Type), .name = "type", .type = cdb.PropType.STR },
                    .{
                        .prop_idx = public.LightCdb.propIdx(.Color),
                        .name = "color",
                        .type = .SUBOBJECT,
                        .type_hash = cetech1.cdb_types.Color3f.type_hash,
                    },
                },
            );
            ...
            const default_light = try _cdb.createObject(db, scale_idx);
...
pub fn load_module_zig(apidb: *const cetech1.apidb.ApiDbAPI, allocator: Allocator, log_api: *const cetech1.log.LogAPI, load: bool, reload: bool) anyerror!bool {
    ...
    _g.light_type_properties_aspec = try apidb.setGlobalVarValue(editor_inspector.UiPropertyAspect, module_name, "ct_light_type_embed_prop_aspect", light_type_aspec);

...
fn compileShaderVariant(
    ...
    // Compile vs shader
    //
    var vs_shader_options = _g.gpu.createDefaultOptionsForRenderer();
    ...
    // Compile fs shader
    //
    var fs_shader_options = _g.gpu.createDefaultOptionsForRenderer();
    ...
    for (main_imports_set.values()) |value| {
        ...
        switch (value.type) {
            ...
            .sampler2d => {
                ...
                try shader.shader_io.samplers.put(_allocator, cetech1.strId32(value.name), .{ .uniform = u, .flags = sampler_flags });
            },
            else => {},
        }
    }
    ...
    if (!_g.gpu.isNoop()) {
        ...
        if (name) |n| {
            ...
            _g.gpu.setShaderName(vs_shader, vs_name.ptr, @intCast(vs_name.len));
            _g.gpu.setShaderName(fs_shader, fs_name.ptr, @intCast(fs_name.len));
        }

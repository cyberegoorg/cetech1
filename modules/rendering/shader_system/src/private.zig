...
const ShaderIO = struct {
    ...
    pub fn clear(self: *ShaderIO) void {
        for (self.uniform_buffer_pool.mem.items[1..self.uniform_buffer_pool.alocated_items.raw]) |*obj| {
            self.uniform_buffer_pool.destroy(obj) catch undefined;
        }

        for (self.resource_buffer_pool.mem.items[1..self.resource_buffer_pool.alocated_items.raw]) |*obj| {
            self.resource_buffer_pool.destroy(obj) catch undefined;
        }
...
const system_context_vt = public.SystemContext.VTable.implement(struct {
    ...
    pub fn getSystem(self: *anyopaque, system: public.System) ?public.SystemInstnace {
        ...
        for (true_buffer.systems) |s| {
            if (s.system.idx != system.idx) continue;
...
fn compileShader(allocator: std.mem.Allocator, use_definitions: []const cetech1.StrId32, definition: ?public.ShaderDefinition, name: ?[:0]const u8) !?public.Shader {
    var new = false;
                ...
                if (!get_or_put.found_existing) {
                    ...
                    if (import.type != .buffer) {
                        if (import.count) |count| {
                            ...
                            try main_imports_w.print(
                                \\void load_{s}(out {s} outval[{d}]) {{
                                \\  outval = {s};
                                \\}}
                                \\
                            , .{ import.name, @tagName(import.type), count, import.name });
                        } else {
...
fn selectShaderVariant(
    ...
    for (contexts) |context| {
        ...
        var context_system_set = public.SystemSet.initEmpty();
        ...
        for (true_system_context.systems[0..true_system_context.count]) |system| {
            context_system_set.set(system.system.idx);
        }
...
var kernel_task = cetech1.kernel.KernelTaskI.implement(
    ...
    struct {
        ...
        pub fn shutdown() !void {
            ...
            for (_g.shader_pool.mem.items[1.._g.shader_pool.alocated_items.raw]) |*obj| {
                obj.deinit(_allocator);
            }

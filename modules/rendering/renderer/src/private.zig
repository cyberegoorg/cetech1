...
const Viewport = struct {
    ...
    fn deinit(self: *Viewport) void {
        ...
        _shader.destroySystemInstance(&self.viewer_system);
        _shader.destroySystemInstance(&self.time_system);
...
const RenderViewportTask = struct {
    ...
    pub fn exec(self: *@This()) !void {
        ...
        if (viewport.world) |world| {
            ...
            var systems = [_]*shader_system.SystemInstance{
                &viewport.time_system,
                &viewport.viewer_system,
            };
            ...
            // TODO: SHIT
            {
                ...
                try self.viewport.time_system.uniforms.?.set(cetech1.strId32("time"), [4]f32{ self.now_s, 0, 0, 0 });
                _shader.submitSystemUniforms(enc, self.viewport.time_system);
            }
            ...
            // Inject main render graph module for shaderables and prepare culling
            {
                ...
                for (impls) |iface| {
                    ...
                    while (q.next(&it)) {
                        ...
                        {
                            var zzz = _profiler.ZoneN(@src(), "Renderer -  Shedarables culling calback");
                            defer zzz.End();

                            rq.mtx.appendSliceAssumeCapacity(t);

                            try iface.prepare_bounding_volumes(
                                allocator,
                                null,
                                t,
                    ...
                    // viewport.all_renderables_counter.* += @floatFromInt(cr.mtx.items.len);
                }
            }
            ...
            // Culling shaderables
            {
                ...
                // First culling phase
                try viewport.shaderables_culling.doCullingSpheres(allocator, viewers.items[0..1]);
                ...
                for (shaderables.keys(), shaderables.values()) |renderable_id, shaderable| {
                    ...
                    try shaderable.prepare_bounding_volumes(
                        allocator,
                ...
                // Last box phase
                try viewport.shaderables_culling.doCullingBox(allocator, viewers.items[0..1]);
            }
            ...
            {
                var z = _profiler.ZoneN(@src(), "Renderer - Build and execute render graph");
                defer z.End();
                ...
                try graph_builder.execute(allocator, fb_size, world, viewers.items);
            }
            ...
            {
                var zz = _profiler.ZoneN(@src(), "Renderer - Culling renderables");
                defer zz.End();
                ...
                // Collect renderables to culling
                {
                    ...
                    for (impls) |iface| {
                        ...
                        while (q.next(&it)) {
                            ...
                            {
                                var zzz = _profiler.ZoneN(@src(), "Renderer - Culling calback");
                                defer zzz.End();
                ...
                // Culling renderables
                {
                    ...
                    const counter = cetech1.metrics.MetricScopedDuration.begin(viewport.culling_duration);
                    defer counter.end();

                    // First culling phase
                    try viewport.renderables_culling.doCullingSpheres(allocator, all_viewers);
                    ...
                    for (renderables.keys(), renderables.values()) |renderable_id, renderable| {
                        ...
                        try renderable.prepare_bounding_volumes(
                            allocator,
                            result.sphere_entites_idx.items,
                            rq.mtx.items,
                            rq.data.items,
                    ...
                    // Last box phase
                    try viewport.renderables_culling.doCullingBox(allocator, all_viewers);
                }
            }
            ...
            {
                var z = _profiler.ZoneN(@src(), "Renderer - Debugdraw pass");
                defer z.End();
                ...
                if (_gpu.getEncoder()) |e| {
                    ...
                    dd.begin(graph_builder.getLayer("color"), true, e);
                    defer dd.end();

                    // Draw culling volumes
                    if (self.viewport.renderables_culling.draw_culling_debug) {

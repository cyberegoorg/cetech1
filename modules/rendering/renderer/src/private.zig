...
const lsd_cube_node_i = graphvm.NodeI.implement(
    .{
        ...
        .category = "Renderer",
        .settings_type = public.TestGeometryNodeSettings.type_hash,
    },
    null,
    struct {
        const Self = @This();
        ...
        pub fn execute(self: *const graphvm.NodeI, args: graphvm.ExecuteArgs, in_pins: graphvm.InPins, out_pins: graphvm.OutPins) !void {
            ...
            const settings_r = public.TestGeometryNodeSettings.read(_cdb, args.settings.?).?;

            const type_str = public.TestGeometryNodeSettings.readStr(_cdb, settings_r, .type) orelse "cube";
            const type_enum = std.meta.stringToEnum(public.TestGeometryNodeType, type_str).?;
...
var create_cdb_types_i = cdb.CreateTypesI.implement(struct {
    pub fn createTypes(db: cdb.DbId) !void {
        ...
        // ConstNodeSettings
        {
            const type_idx = try _cdb.addType(
                db,
                public.TestGeometryNodeSettings.name,
                &[_]cdb.PropDef{
                    .{
                        .prop_idx = public.TestGeometryNodeSettings.propIdx(.type),
                        .name = "type",
                        .type = .STR,
                    },
                },

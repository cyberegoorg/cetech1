...
pub const CullingRequest = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, data_size: usize, cullable_count: usize) !CullingRequest {
        ...
        try cr.mtx.ensureTotalCapacity(cr.allocator, cullable_count);
    ...
    pub fn clear(self: *CullingRequest, cullable_count: usize) !void {
        ...
        try self.mtx.ensureTotalCapacity(self.allocator, cullable_count);
...
pub const CullingResult = struct {
    ...
    visibility: VisibilityBitFieldList = .{},

    sphere_entites_idx: EntitiesIdxList = .{},
    box_entites_idx: EntitiesIdxList = .{},
    compact_visibility: VisibilityBitFieldList = .{},
    ...
    pub fn setVisibility(self: *CullingResult, idx: usize, viewer_idx: usize, visible: bool) void {
        self.visibility.items[idx].setValue(viewer_idx, visible);
    }
    ...
    pub fn compaction(self: *CullingResult, count: usize, out_entites_idx: []usize, in_entites_idx: ?[]const usize) void {
        var renderable_cnt: usize = 0;
        for (0..count) |cullable_idx| {
            if (self.visibility.items[cullable_idx].count() == 0) continue;
    ...
    pub fn compactionSpheres(self: *CullingResult, count: usize) !void {
    ...
    pub fn compactionBox(self: *CullingResult, count: usize) !void {
...
const CullingSphereTask = struct {
    ...
    pub fn exec(self: *@This()) !void {
        ...
        for (self.volumes, 0..) |sphere, i| {
            ...
            for (self.frustums, 0..) |frustum, frustum_idx| {
                if (cetech1.math.frustumPlanesVsSphereNaive(frustum, sphere.center, sphere.radius)) {
                    self.result.setVisibility(self.visibility_offset + i, frustum_idx, true);
                    any_visible = true;
                }
            }
...
const CullingBoxTask = struct {
    ...
    pub fn exec(self: *@This()) !void {
        ...
        for (self.volumes, 0..) |box, i| {
            ...
            for (self.frustums, 0..) |frustum, frustum_idx| {
                if (cetech1.math.frustumPlanesVsOBBNaive(frustum, box.t.mtx, box.min, box.max)) {
                    self.result.setVisibility(self.visibility_offset + i, frustum_idx, true);
                    any_visible = true;
                }
            }
...
pub const CullingSystem = struct {
    ...
    pub fn doCullingSpheres(self: *Self, allocator: std.mem.Allocator, viewers: []const render_graph.Viewer) !void {
        ...
        //
        {
            ...
            for (self.request_map.keys(), self.request_map.values()) |k, value| {
                ...
                const items_count = value.sphere_volumes.items.len;

                try result.visibility.resize(result.allocator, items_count);
                @memset(result.visibility.items, public.VisibilityBitField.initEmpty());
        ...
        // Filter sphere results
        {
            ...
            for (self.request_map.keys(), self.request_map.values()) |k, rq| {
                const result = self.getResult(k) orelse continue; // TODO: warning
                try result.compactionSpheres(rq.sphere_volumes.items.len);
            }
        }
    }

    pub fn doCullingBox(self: *Self, allocator: std.mem.Allocator, viewers: []const render_graph.Viewer) !void {
        ...
        // Filter box results
        {
            ...
            for (self.request_map.keys(), self.request_map.values()) |k, rq| {
                const result = self.getResult(k) orelse continue; // TODO: warning
                try result.compactionBox(rq.box_volumes.items.len);
            }
        }
    }

    pub fn debugdrawBoundingBoxes(self: *Self, dd: gpu.DDEncoder) !void {
        var zone = self.profiler.ZoneN(@src(), "Culling system - Draw debug");
        defer zone.End();

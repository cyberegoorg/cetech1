...
const camera_controller_system_i = ecs.SystemI.implement(
    .{
        ...
        .query = &.{
            ...
            .{ .id = ecs.id(transform.Rotation), .inout = .InOut },
    ...
    struct {
        pub fn update(world: ecs.World, it: *ecs.Iter, dt: f32) !void {
            ...
            const pos = it.field(transform.Position, 0).?;
            const rot = it.field(transform.Rotation, 1).?;
            const cams = it.field(camera.Camera, 2).?;
            const controllers = it.field(public.CameraController, 3).?;
            ...
            for (0..it.count()) |i| {
                ...
                if (camera_look_activated_new) {
                    ...
                    var py = zm.quatToRollPitchYaw(rot[i].q);
                    ...
                    // Look handle
                    {
                        ...
                        rot[i].q = zm.quatFromRollPitchYaw(py[0], py[1], 0);
                    }
                    ...
                    // Move handle
                    {
                        ...
                        var cam_pos = zm.loadArr3(.{ pos[i].x, pos[i].y, pos[i].z });

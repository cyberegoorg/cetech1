...
const RenderComponentTask = struct {
    ...
    volumes: []const ?*renderer.CullingVolume,
    ...
    pub fn exec(self: *@This()) !void {
        ...
        if (_gpu.getEncoder()) |e| {
            ...
            for (self.volumes, self.draw_calls, self.transforms) |culling_volume, draw_call, mtx| {
                if (true) {
                    if (draw_call) |dc| {
                            ...
                            for (self.viewers) |viewver| {
                                if (_shader_system.selectShaderVariant(
                                    ...
                                    if (variant.prg) |prg| {
                                        ...
                                        const layer = if (variant.layer) |l| self.builder.getLayerById(l) else 256; // TODO: SHIT
        ...
        if (culling_result) |result| {
            ...
            if (try cetech1.task.batchWorkloadTask(
                .{
                    ...
                    .batch_size = draw_calls.len, // TODO: flickering
                },
                ARGS{
                    .viewport = viewport,
                    .viewers = viewers,
                ...
                struct {
                    pub fn createTask(create_args: ARGS, batch_id: usize, args: cetech1.task.BatchWorkloadArgs, count: usize) RenderComponentTask {
                        ...
                        return RenderComponentTask{
                            ...
                            .transforms = create_args.transforms[batch_id * args.batch_size .. (batch_id * args.batch_size) + args.batch_size],
                            .volumes = create_args.volumes[batch_id * args.batch_size .. (batch_id * args.batch_size) + args.batch_size],
                            .draw_calls = create_args.draw_calls[batch_id * args.batch_size .. (batch_id * args.batch_size) + args.batch_size],
                        };
                    }
                },
            )) |t| {

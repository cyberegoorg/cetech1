...
const vertex_system_data_strid = cetech1.strId32("vertex_system_data");
...
fn createVertexSystemFromVertexBuffer(vertex_buffer: public.VertexBuffer) !public.GPUGeometry {
    ...
    var channel_offset: [4]zm.Vec = @splat(@splat(0));
    var channel_stride: [4]zm.Vec = @splat(@splat(0));
    var channel_buffer_idx: [4]zm.Vec = @splat(@splat(0));
    while (it.next()) |channel_id| {
        channel_offset[channel_id / 4][channel_id % 4] = @bitCast(vertex_buffer.channels[channel_id].offset);
        channel_stride[channel_id / 4][channel_id % 4] = @bitCast(vertex_buffer.channels[channel_id].stride);
        channel_buffer_idx[channel_id / 4][channel_id % 4] = @bitCast(vertex_buffer.channels[channel_id].buffer_idx);
    }

    try _shader_system.updateUniforms(system_io, vertex_system_uniforms, &.{
        .{
            .name = vertex_system_data_strid,
    ...
    for (vertex_buffer.buffers, 0..) |vb, idx| {
        switch (idx) {
            0 => try _shader_system.updateResources(system_io, vertex_system_resources, &.{.{ .name = vertex_system_channel0_strid, .value = .{ .vb = vb } }}),
            1 => try _shader_system.updateResources(system_io, vertex_system_resources, &.{.{ .name = vertex_system_channel1_strid, .value = .{ .vb = vb } }}),
            else => {},
        }
    }
    ...
    struct {
        pub fn init() !void {
            ...
            try _shader_system.addSystemDefiniton(
                ...
                .{
                    .imports = &.{
                        .{ .name = "vertex_system_data", .type = .vec4 },

                        .{ .name = "vertex_system_offsets", .type = .vec4, .count = 4 },
                        .{ .name = "vertex_system_strides", .type = .vec4, .count = 4 },
                        .{ .name = "vertex_system_buffer_idx", .type = .vec4, .count = 4 },
                    ...
                    .vertex_block = .{
                        ...
                        .common_block = @embedFile("shaders/vs_common_block.sc"),
                        .code =

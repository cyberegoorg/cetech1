...
fn createVertexSystemFromVertexBuffer(allocator: std.mem.Allocator, vertex_buffer: public.VertexBuffer) !public.GPUGeometry {
    ...
    for (buffer_idx_map.keys(), 0..) |vb, idx| {
        switch (idx) {
            0 => try _shader_system.updateResources(system_io, vertex_system_resources, &.{.{ .name = vertex_system_channel0_strid, .value = vb }}),
            1 => try _shader_system.updateResources(system_io, vertex_system_resources, &.{.{ .name = vertex_system_channel1_strid, .value = vb }}),
            else => {},
        }
    }
        ...
        pub fn init() !void {
            ...
            try _shader_system.addSystemDefiniton(
                ...
                .{
                    ...
                    .common_block = @embedFile("shaders/common_block.sc"),

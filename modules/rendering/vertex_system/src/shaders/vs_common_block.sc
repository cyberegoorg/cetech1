...
void init_vertex_loader(out ct_vertex_loader_ctx ctx, in ct_input input, in uint instance_id) {
...
vec4 load_vertex_position(in ct_vertex_loader_ctx ctx, in uint vertex_id, in uint set) {
    uint offset = (set * ctx.num_vertices + vertex_id) * floatBitsToUint(read_by_idx(ctx.stride, CT_VERTEX_SEMANTIC_POSITION)) + floatBitsToUint(read_by_idx(ctx.offset, CT_VERTEX_SEMANTIC_POSITION));
    uint buffer_idx = floatBitsToUint(read_by_idx(ctx.buffer_idx, CT_VERTEX_SEMANTIC_POSITION));
...
vec4 load_vertex_color0(in ct_vertex_loader_ctx ctx, in uint vertex_id, in uint set) {
    uint offset = (set * ctx.num_vertices + vertex_id) * floatBitsToUint(read_by_idx(ctx.stride, CT_VERTEX_SEMANTIC_COLOR0)) + floatBitsToUint(read_by_idx(ctx.offset, CT_VERTEX_SEMANTIC_COLOR0));
    uint buffer_idx = floatBitsToUint(read_by_idx(ctx.buffer_idx, CT_VERTEX_SEMANTIC_COLOR0));

...
pub fn ratioFromEnum(ratio: public.BackbufferRatio) f32 {
    return switch (ratio) {
        .Equal => 1.0,
        .Half => 1.0 / 2.0,
        .Quarter => 1.0 / 4.0,
...
const GraphBuilder = struct {
    ...
    pub fn enablePass(self: *GraphBuilder, pass: *public.Pass, name: []const u8) !void {
    ...
    pub fn execute(self: *GraphBuilder, allocator: std.mem.Allocator, vp_size: [2]f32, viewers: []const public.Viewer) !void {
        ...
        for (self.dag.output.keys()) |pass| {
            ...
            try pass.api.execute(pass, builder, _gpu, vp_size, info.viewid);
        }
    }

...
const GraphBuilder = struct {
    ...
    pub fn execute(self: *GraphBuilder, vp_size: [2]f32, world: ecs.World, viewers: []const public.Viewer) !void {
...
pub fn createBuilder(allocator: std.mem.Allocator) !public.GraphBuilder {
    ...
    new_builder.* = GraphBuilder.init(allocator);
...
pub const builder_vt = public.GraphBuilder.VTable{
    ...
    .reset = @ptrCast(&GraphBuilder.clear),
};

...
const GraphBuilder = struct {
    ...
    pub fn execute(self: *GraphBuilder, allocator: std.mem.Allocator, vp_size: [2]f32, viewers: []const public.Viewer) !void {
        ...
        for (self.dag.output.keys()) |pass| {
            ...
            if (info.needFb()) {
                ...
                const fb = self.gpu.createFrameBufferFromHandles(@truncate(textures.items.len), textures.items.ptr, false);
                self.gpu.setFrameBufferName(fb, info.name.ptr, @intCast(info.name.len));

                info.fb = fb;

                self.gpu.setViewName(info.viewid, info.name.ptr, @intCast(info.name.len));
    ...
    pub fn getOrCreateTexture2D(self: *GraphBuilder, vp_size: [2]f32, texture_name: []const u8, info: public.TextureInfo) !gpu.TextureHandle {
        ...
        {
            self.gpu.setTextureName(t, texture_name.ptr, @intCast(texture_name.len));

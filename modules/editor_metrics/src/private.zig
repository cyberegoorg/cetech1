...
const SelectedMetrics = cetech1.mem.HashSet([]const u8);
// Struct for tab type
...
var foo_tab = editor.EditorTabTypeI.implement(editor.EditorTabTypeIArgs{
    ...
    pub fn ui(inst: *editor.TabO, kernel_tick: u64, dt: f32) !void {
        ...
        if (_coreui.beginPlot("Metrics", .{
            ...
            while (it.next()) |v| {
                var split_bw = std.mem.splitBackwards(u8, v.*, "/");
                ...
                if (_metrics.getMetricValues(allocator, v.*)) |values| {
                    _coreui.plotLineValuesF64(name, .{
                        .v = values,
                        .offset = @intCast(_metrics.getMetricOffset(v.*).?),
                    });
                }
            }
        }
    ...
    pub fn menu(inst: *editor.TabO) !void {
        ...
        if (_coreui.beginMenu(_allocator, coreui.Icons.Remove, true, null)) {
            ...
            var it = tab_o.selected_metrics.iterator();
            while (it.next()) |v| {
                const name = try std.fmt.bufPrintZ(&buf, "{s}###{s}", .{ v.*, v.* });
                if (_coreui.menuItem(_allocator, name, .{}, null)) {
                    _ = tab_o.selected_metrics.remove(v.*);
                }
            }
        }
    }
...
pub export fn ct_load_module_editor_metrics(__apidb: *const cetech1.apidb.ApiDbAPI, __allocator: *const std.mem.Allocator, __load: bool, __reload: bool) callconv(.C) bool {

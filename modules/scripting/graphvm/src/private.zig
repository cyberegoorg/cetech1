...
const GraphVM = struct {
    ...
    used_instance_set: UsedInstnaceSet,
    ...
    pub fn init(allocator: std.mem.Allocator, graph: cdb.ObjId) !Self {
        return Self{
            ...
            .transpile_state_map = .{},
            .transpile_map = .{},
            .used_instance_set = try UsedInstnaceSet.initEmpty(allocator, MAX_VM_INSTANCE),
    ...
    pub fn deinit(self: *Self) void {
        ...
        for (self.instance_pool.mem.items[1..self.instance_pool.alocated_items.raw]) |*obj| {
            obj.deinit(self);
        }
        ...
        for (self.vmnodes.mem.items[1..self.vmnodes.alocated_items.raw]) |*node| {
            node.deinit();
        }
        ...
        self.used_instance_set.deinit(self.allocator);
        self.used_nodes_set.deinit(self.allocator);
    }

    pub fn clean(self: *Self) !void {
        for (self.instance_pool.mem.items[1..self.instance_pool.alocated_items.raw]) |*value| {
            try value.clean(0, 0);
        }
        ...
        // self.used_instance_set.unsetAll();
    }
    ...
    pub fn buildVM(self: *Self, allocator: std.mem.Allocator) !void {
        ...
        // const data_maps = self.vmnodes.mem.items(.data_map);
        // const pin_def = self.vmnodes.mem.items(.pin_def);
        // const output_blob_size = self.vmnodes.mem.items(.output_blob_size);
        ...
        // TODO: SHIT
        {
            ...
            // add all nodes
            for (0..self.vmnodes.alocated_items.raw) |node_idx| {
        ...
        if (self.instance_pool.alocated_items.raw != 0) {
            ...
            for (self.instance_pool.mem.items[1..self.instance_pool.alocated_items.raw], 0..) |*value, idx| {
                if (!self.used_instance_set.isSet(idx + 1)) continue;
                instances[inst_count] = value;
                inst_count += 1;
            }
        ...
        while (deleted_it.next()) |entry| {
            ...
            try self.vmnodes.destroy(vmnode);
    ...
    pub fn createInstances(self: *Self, allocator: std.mem.Allocator, count: usize) ![]*VMInstance {
...

        {
            for (0..count) |idx| {
                ...
                const instance_idx = self.instance_pool.index(instance);
                self.used_instance_set.set(instance_idx);
    ...
    pub fn destroyInstance(self: *Self, instance: *VMInstance) void {
        self.used_instance_set.unset(self.instance_pool.index(instance));
        self.instance_pool.destroy(instance) catch undefined;
    }

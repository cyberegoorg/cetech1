...
const init_logic_system_i = ecs.SystemI.implement(
    ...
    struct {
        ...
        //     const logic_components = it.field(public.GraphVMLogicComponent, 1).?;

        //     const instances = try alloc.alloc(graphvm.GraphInstance, logic_components.len);
        //     defer alloc.free(instances);

        //     // TODO: SHIT
        //     if (logic_components[0].graph.isEmpty()) return;

        //     try _graphvm.createInstances(alloc, logic_components[0].graph, instances);
...
const tick_logic_system_i = ecs.SystemI.implement(
    ...
    struct {
        pub fn tick(world: ecs.World, it: *ecs.Iter, dt: f32) !void {
            ...
            // world.deferSuspend();
            if (all_instances.items.len > 0) {
                // log.debug("Tick {d}", .{all_instances.items.len});
                try _graphvm.executeNode(alloc, all_instances.items, graphvm.EVENT_TICK_NODE_TYPE, .{ .use_tasks = true });
            }
            // world.deferResume();
        }
    },

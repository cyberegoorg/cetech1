...
const culling_volume_node_i = graphvm.NodeI.implement(
    ...
    struct {
        ...
        pub fn execute(self: *const graphvm.NodeI, args: graphvm.ExecuteArgs, in_pins: graphvm.InPins, out_pins: *graphvm.OutPins) !void {
            ...
            _, const min: math.Vec3f = in_pins.read(math.Vec3f, 1) orelse .{ 0, .{} };
            _, const max: math.Vec3f = in_pins.read(math.Vec3f, 2) orelse .{ 0, .{} };
...
const draw_call_node_i = graphvm.NodeI.implement(
    .{
        ...
        .pivot = .pivot,
        .category = "Renderer",
        .settings_type = public.DrawCallNodeSettingsCdb.type_hash,
    },
    public.DrawCall,
    struct {
        const Self = @This();
        ...
        pub fn execute(self: *const graphvm.NodeI, args: graphvm.ExecuteArgs, in_pins: graphvm.InPins, out_pins: *graphvm.OutPins) !void {
            ...
            const flags_obj = public.DrawCallNodeSettingsCdb.readSubObj(_cdb, settings_r, .visibility_flags).?;
...
const simple_mesh_node_i = graphvm.NodeI.implement(
    .{
        ...
        .category = "Renderer",
        .settings_type = public.SimpleMeshNodeSettingsCdb.type_hash,
    },
    null,
    struct {
        const Self = @This();
        ...
        pub fn execute(self: *const graphvm.NodeI, args: graphvm.ExecuteArgs, in_pins: graphvm.InPins, out_pins: *graphvm.OutPins) !void {
            ...
            const type_str = public.SimpleMeshNodeSettingsCdb.readStr(_cdb, settings_r, .type) orelse "cube";
    ...
    pub fn createTypes(db: cdb.DbId) !void {
        ...
        // ConstNodeSettings
        {
            const type_idx = try _cdb.addType(
                ...
                &[_]cdb.PropDef{
                    .{
                        .prop_idx = public.SimpleMeshNodeSettingsCdb.propIdx(.type),
                        .name = "type",
                        .type = .STR,
                    },
                },
        ...
        // DrawCallNodeSettings
        {
            draw_call_setttings_type_idx = try _cdb.addType(
                ...
                &[_]cdb.PropDef{
                    .{
                        .prop_idx = public.DrawCallNodeSettingsCdb.propIdx(.visibility_flags),
...
const post_create_types_i = cdb.PostCreateTypesI.implement(struct {
    pub fn postCreateTypes(db: cdb.DbId) !void {
        ...
        try public.DrawCallNodeSettingsCdb.setSubObj(_cdb, default_settings_w, .visibility_flags, default_flags_w);

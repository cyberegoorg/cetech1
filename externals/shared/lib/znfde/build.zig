    ...
    switch (lib.rootModuleTarget().os.tag) {
        ...
        .macos => {
            ...
            if (options.with_portal) {
                lib.addSystemIncludePath(.{ .path = "includes" });
                znfde.addSystemIncludePath(.{ .path = "includes" });
                lib.addCSourceFile(.{ .file = .{ .path = "nativefiledialog/src/nfd_portal.cpp" }, .flags = &cflags });
                lib.linkSystemLibrary("dbus-1");
            } else {
                lib.addCSourceFile(.{ .file = .{ .path = "nativefiledialog/src/nfd_gtk.cpp" }, .flags = &cflags });

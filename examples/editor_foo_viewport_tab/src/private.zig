...
const move_system_i = ecs.SystemI.implement(
    ...
    struct {
        pub fn update(iter: *ecs.IterO) !void {
            ...
            const p = it.field(transform.Position, 1).?;
...
var foo_tab = editor.EditorTabTypeI.implement(editor.EditorTabTypeIArgs{
    ...
    pub fn create(db: cdb.Db) !?*editor.EditorTabI {
        ...
        tab_inst.* = .{
            .viewport = try _viewport.createViewport(_g.rg, w),
            .world = w,
            .tab_i = .{
        ...
        if (w.newEntities(allocator, SPHERE_COUNT)) |entities| {
            ...
            for (entities) |ent| {
                ...
                //     ent,
                //     &transform.Rotation{
                //         .q = zm.quatFromRollPitchYaw(10, 10, 10),
                //     },
                // );
                ...
                if (graph_obj) |obj| {
                    const graph_inst = try _graph.createInstance(db, obj);

                    try _graph.setInstanceContext(graph_inst, ECS_WORLD_CONTEXT, tab_inst.world.ptr);
                    try _graph.setInstanceContext(graph_inst, ECS_ENTITY_CONTEXT, @ptrFromInt(ent));

                    _ = w.setId(renderer.RenderComponent, ent, &renderer.RenderComponent{
                        .graph_container = graph_inst,
                    });
                }
            }
        }
...
const SimpleFPSCamera = struct {
    ...
    inline fn calcForward(self: *SimpleFPSCamera) void {
        ...
        const forward = zm.normalize3(zm.mul(zm.f32x4(0.0, 0.0, 1.0, 0.0), t));
